...
2) 가로(x)축과 세로(y)축을 이동하며 for문이 진행되며 좌표마다 +1을 하는데 두번 만나면 보라색   
   (파란색(1)이 채워진 좌표를, 빨간색이 때 또 오게되면 보라색이 되는 것임)
...
for tc in range(1, T+1):
    ...
    for case in range(1 << 12): # 1 << 12 는 4096 (2의 12승)
        ...
        for i in range(12): 
            if case & (1 << i): # 아래 참고
                sub_sum.append(nums[i]) # sub_sum에 nums[i]을 리스트로 추가하고 
                sum_value += nums[i] # sum_value 에도 num[i] 값 추가 
        if len(sub_sum) == n and sum_value = k:
            cnt += 1 # 부분집합 원소갯수가 n개 이고 부분집합의 합이 k 면 cnt 증가
    print(f'#{tc} {cnt}')
            
```

### if case & (1 << i) 의 의미 (비트연산자의 활용!)
