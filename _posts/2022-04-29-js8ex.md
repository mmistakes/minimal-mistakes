...
When I execute `signUp()`, it will make an array consisted of usernames, and check if the input from `prompt` is in the array. If not, it'll call `signIn()` function.<br>

The `signIn()` function creates a new object, and assign the key-value pairs. I wanted the `_id` to be randomly assigned by a function, so I made `generateId()` function, which generates 6 digits of random ID.
<br>

The `generateTime()` function assigns the value to `createdAt` property. I set the value of `isLoggedIn` property as `true` for default.
<br>
