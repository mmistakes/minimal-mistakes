...
<br>
<span style = "font-weight:bold;">
1) 삭제 대상의 노드가 유일한 노드인 경우</span>:링크드 리스트의 노드가 1개밖에 안 남은 경우에는 해당 노드가 head 노드이자 동시에 tail 노드이고 해당 노드를 삭제하면 링크드 리스트가 완전히 비게 된다. 이 경우 head와 tail 노드를 아예 None으로 없애 버리면 자연스럽게 해당 노드가 삭제되어 링크드 리스트가 완전히 비게 된다.
...
<br>
<span style = "font-weight:bold;">
2) 노드가 2개 이상의 링크드 리스트의 맨 뒤를 삭제하는 경우</span>: 이 경우는 삭제하려고 하는 노드가 tail 노드이기 때문에 tail 노드의 바로 앞 노드(previous node)의 next를 None으로 바꾸어 버리면 자연스럽게 삭제하고자 했던 기존의 tail 노드는 링크드 리스트에서 아웃된다. 이후 previous node를 새로운 tail 노드로 지정한다.
...
<br>
<span style = "font-weight:bold;">
2) 노드가 2개 이상의 링크드 리스트의 맨 앞을 삭제하는 경우</span>: 삭제하려는 노드가 맨 앞의 head 노드인 경우 기존의 head 노드의 next 값을 None으로 바꾸고 다음 노드를 새로운 head 노드로 지정해주면 기존의 head 노드는 자연스럽게 링크드 리스트와 연결이 끊겨 삭제된다.
...
<br>
<span style = "font-weight:bold;">
2) 노드가 2개 이상의 링크드 리스트의 중간을 삭제하는 경우(두 노드 사이의 노드를 삭제)</span>:
