...
React Query는 API호출과 비동기 데이터 관리에도 적극적으로 활용되고 있다. <br />
다른 라이브러리들과 비교해서 왜 효율이 좋은지 알기 위해, 이전에는 다른 라이브러리를 사용해서 어떻게 데이터를 다뤘는지 알아보자.
...
프로젝트 상황에 맞춰 redux-thunk, redux-saga 등 적절한 미들웨어를 사용하며 업무 효율을 높였다.
