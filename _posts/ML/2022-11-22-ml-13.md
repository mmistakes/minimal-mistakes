...
현재(```Wcurrent```) 위치에서 가장 Loss가 작은 지점(```Wbest```)로 나아가는 과정이다. 함수가 복잡한데 Gradient를 구하고 Gradient 방향으로 해를 계속 업데이트함으로써 최적의 해를 찾는다. 

하지만 함수가 복잡하기 때문에 우리가 알 수 없다. Loss 함수가 이차함수라면 미분값이 0인 지점을 기준으로 최소값을 찾을 수 있지만 다음과 같은 함수가 있을 수 있다.
...
3차원 그래프(왼쪽)를 그리게 되면 안보이는 부분이 많이 생긴다는 단점이 있다. 또한 위 그림과 같이 미분값이 0인 지점이 여러개 있기 때문에 전역적인 최적해(Global Optimum)를 구할 수 있다는 보장이 어렵다.

이때 3차원 그래프의 단점을 보완하기 위해 3차원 정보를 2차원인 등고선(오른쪽)으로 표현한다. 각 영역마다 Loss가 정의되는데 가장 작은 Loss 를 찾는 과정이다.
...
최솟값은 global minimum이지만 출발 시점에 따라 local minimum 때문에 모델 성능이 나빠질 수 있다. 그러나 MSE의 경우 볼록 함수(convex function)로 local minimun에 빠질 가능성이 없으며, 고차원의 공간에서는 local minimum에 빠질 가능성이 현저히 낮으므로 큰 문제가 되지 않는다.
...
- [Github 블로그 angeloyeo - 경사하강법(gradient descent)](https://angeloyeo.github.io/2020/08/16/gradient_descent.html)
