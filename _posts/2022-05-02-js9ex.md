...
+ **median** : the middle value when a data set is sorted. If the length of a set is an odd number, it's the middle value. If not, it's an average of two values in the middle.
...
function calculateStatistics(arr) {
  const statistics = {};
  statistics.count = arr.length;
  statistics.sum = arr.reduce((a, b) => a + b);
  statistics.min= Math.min(...arr);
  statistics.max = Math.max(...arr);
  statistics.range = Math.max(...arr) - Math.min(...arr);
  statistics.mean = statistics.sum / statistics.count;

  //median
  const sortedArr = arr.sort((a, b) => a - b);
  if (arr.length % 2 != 0) {
    statistics.median = sortedArr[(arr.length - 1) / 2];
  } else {
    statistics.median = (sortedArr[(arr.length / 2) - 1] + sortedArr[arr.length / 2]) / 2;
  }
  ...
  statistics.mode = num[count.indexOf(max)];

  //variance
  const deviation = arr.map((element) => element - statistics.mean)
  const square = deviation.map((num) => num * num);
  const sum = square.reduce((a, b) => a + b)
  statistics.variance = sum / statistics.count
...
/*
{count: 25, sum: 744, min: 24, max: 38, range: 14,
 mean: 29.76, median: 29, mode: 26}
*/
```
