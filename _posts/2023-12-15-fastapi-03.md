---
layout: single
title: "FastAPI - 03 (FastAPI: 쿼리(Query) 파라미터)"
categories: FastAPI
...
경로 파라미터가 아닌 함수의 매개변수는 FastAPI에서 자동으로 쿼리(Query) 파라미터로 인식됩니다. 이는 함수에 선언된 매개변수가 URL의 경로 일부가 아니라면, 그 매개변수가 URL의 쿼리 스트링으로 처리된다는 의미입니다.
...
`skip`, `limit`는 경로 파라미터에 선언 되지 않았으므로 쿼리 파라미터로 처리 됩니다.  
...
'item_id'는 경로 파라미터로 'foo'가 설정 되고, 'q'는 쿼리 파라미터로 3이 설정되어 `return {"item_id": 'foo', "q": '3'}`으로 처리 됩니다. 결과는 다음과 같습니다.
...
'item_id'는 경로 파라미터로 'foo'가 설정 되고, 'q'는 None이 설정되어 `return {"item_id": 'foo'}`으로 처리 됩니다. 결과는 다음과 같습니다.
...
쿼리 파라미터를 bool 타입으로 설정 할 수 있으며 여러 유형의 값으로 bool 타입으로 변경 됩니다.
...
FastAPI는 함수에 선언된 매개변수의 이름을 통해 경로 파라미터와 쿼리 파라미터를 자동으로 구분합니다. 이를 통해 여러 경로 및 쿼리 파라미터를 하나의 함수에서 효과적으로 처리할 수 있습니다.
...
@app.get("/users/{user_id}/items/{item_id}")  # 2개의 경로 파라미터(user_id, item_id)
...
쿼리 파라미터를 필수로 설정하려면, 기본값(None 포함)을 설정하지 않으면 됩니다.
...
@app.get("/items/{item_id}")
# needy에 기본 값을 설정 하지 않음 -> 필수 쿼리 파라미터
