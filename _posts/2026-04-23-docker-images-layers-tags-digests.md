...
- 실행 결과 요약: `docker` CLI가 없어 `docker pull`, `docker image tag`, `docker image inspect` 예제를 현재 환경에서 직접 재현하지 못했다.
- 직접 재현 없음: 따라서 이 글의 명령 예시와 구분 설명은 Docker 공식 문서가 제시하는 CLI 예시와 설명을 기준으로 정리했다.
...
현재 작성 환경에는 Docker CLI가 없어 실제 `pull`, `tag`, `inspect` 결과를 직접 붙이지 못했다. 따라서 이 글의 사실 섹션은 2026-04-20 기준 Docker 공식 문서가 직접 설명하는 동작에 한정한다.
