...
license: Taha Monfared
---
{% include toc icon="gears" title="Table of contents" %}

To solve this problem we need to partition our data. Each observation should be sorted such that missing values are at the top of the observation vector. $X=\left[\begin{array}{c}X_{m} \\ \hline X_{o}\end{array}\right]$, where"o" stands as observed and "m" stands as missing.
...
Now we are ready to start our EM algorithm. First we assume we have all the data.Therefore, our likelihood becomes $L_c(\mu,\Sigma|X)=\prod_{i=1}^{n}(2\pi)^{-p/2}|\Sigma|^{-1/2}exp\{-1/2(x-\mu)^T\Sigma^{-1}(x-\mu)\}$.Where "c" stands for complete data. 
