...
- 이미 만들었던 게시판 CRUD에서 회원가입과 로그인 API를 추가하고 인증 미들웨어를 만들어서 jwt를 검증하여 로그인한 회원이 게시물을 작성하고 댓글을 작성할 수 있도록 수정하였다.
  <br>확실히 첫번째 프로젝트에서 겪은 고통이 어마어마했어서 그런지 이번 프로젝트는 생각보다 무난하게 느껴졌다. 그리고 이번 프로젝트를 진행함에 있어서 저번 프로젝트에서 깨달았던 점을 활용하여 해결 순서를 세분화하여 로직을 짜기위해 더 노력했던 것 같다.
  <br>하지만 아직도 한참 부족하다. 비즈니스 로직짜는게 왜이렇게 힘든지... 그리고 두번째 과제를 진행하며 느낀점이 있는데, 아직 기본기가 많이 부족한 것 같다. 앞으로 과제에 온 힘을 다하는 것도 좋지만, 따로 시간을 할당하여 알고리즘과 Javascript 언어에 대해서 더 익숙해지고 더 효율적으로 공부할 필요성이 있는것 같다.
  <br>프로젝트들을 진행하면서 느낀 나의 부족한 점들을 한번에 고칠 수 있을거라고는 생각하지 않는다. 하지만 꾸준히 노력하다보면 언젠가 나의 부족한 부분들을 고쳐나갈 수 있지 않을까?
