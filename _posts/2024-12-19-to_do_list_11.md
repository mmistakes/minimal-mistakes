...
```js
import React, { useState, useEffect } from 'react'; // React 라이브러리에서 필요한 기능 가져오기
import './App.css'; // 스타일을 적용하기 위한 CSS 파일 가져오기

function App() {
  const [todos, setTodos] = useState([]); // 할 일 목록 상태 관리

  // 컴포넌트가 처음 렌더링될 때 LocalStorage에서 데이터를 불러옴
  useEffect(() => {
    ...
    <div className="container"> {/* 전체 UI를 감싸는 컨테이너 */}
      ...
      <form
        onSubmit={(e) => {
          ...
          const input = e.target.elements.todoInput; // 폼 내 입력 필드 참조
          addTodo(input.value); // 입력된 값을 기반으로 새로운 할 일 추가
