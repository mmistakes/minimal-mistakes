...
if (dlines > 0) // 지워진 라인이 한 개 이상이면
{
    game_score += (dlines - 1) * 4;   // 추가 점수
    deleted_lines += dlines;           // 지워진 라인 수 누적
    game_level = deleted_lines / 4 + 1; // 레벨 계산 (4라인마다 1레벨 증가)
    if (game_level > 9)
        game_level = 9;               // 최대 레벨 제한
    block_delay = 500 - (50 * (game_level - 1)); // 블록 떨어지는 속도 조절
    print_score(game_score);          // 점수 출력
    print_level(game_level);          // 레벨 출력

    if (game_high_score < game_score)
    {
        game_high_score = game_score;
    }
    print_high_score(game_high_score); // 최고 점수 출력
}
```
...
```c
for (i = 1; i <= minute; i++) {
    if (game_score <= i * 100) {     // 제한 시간(분)마다 목표 점수 체크
        textcolor(12);
        print_at(8, 10, "※ GAME OVER ※");
        state = GAMEOVER;            // 게임 종료 상태로 변경
        break;
    }
}
```
