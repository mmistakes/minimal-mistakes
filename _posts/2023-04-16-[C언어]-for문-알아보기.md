...
```
이렇게 코드를 짜면 `1`부터 `100`까지의 숫자 중에 4의 배수에 해당하는 값만 출력을 해준다. `for`문은 아까와 똑같으니 넘어가보면 `if (i%4 != 0){continue;}`가 있는데, `i`를 `4`로 나눈 나머지가 `0`이 아닌 경우는 `continue;`, 즉 넘어가라는 뜻이다. `continue;`는 `for`문을 끝내
<!--stackedit_data:
eyJoaXN0b3J5IjpbMzY3NjA2OTk1LDE4MTU2ODc3NzcsMTMyOT
kxNDkzNywxNDM3NjE5ODY3XX0=
-->