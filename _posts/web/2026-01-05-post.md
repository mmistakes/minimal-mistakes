...
- 예시코드
    ...
    def sync_view(request): // 클라이언트 요청 들어오면 실행
        response = requests.get("https://api.example.com/data")
        data = response.json() // 응답 받으면 데이터 파싱
        return JsonResponse(data)
    ```
    - requests.get()을 호출 하여 외부 API에 요청을 보내고 응답이 올때까지 스레드는 멈춰있는 상태임 -> 동기 방식
    - Blocking + Sync 조합의 코드 예시
        - requests.get()은 Blocking 관점에서 보면 Blocking I/O임
...
- 예시코드
    ...
    async def async_view(request): // 요청 들어오면 실행
    ...
    ```
    - client.get()으로 외부 API 요청을 시작하지만
    - await 만나면서 현재 작업은 이벤트 루프에 제어권을 넘기고, 스레드는 멈추지 않고 다른 작업을 처리 가능한 상태
    - Non-blocking + Async 조합의 대표적인 예시
        - await client.get()은 Non-blocking 관점에서 보면 Non-blocking임
