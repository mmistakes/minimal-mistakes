...
&&A_1$$이라는 특정 사건이 발생할 확률에 대해서 알아보자, 이는 그림에서도 봤듯이 A라는 전체집합과 

&&A_1$$ 집합의 교집합이다. 이것을 조건부 확률로 나타낼 수도 있는데 특정 사건 &&A_1$$이 발생했을 때 A가 발생할 확률에다가,  &&A_1$$이 동시에 발생해야 함으로 곱으로 나타낼 수 있다. 
