...
Everything can be an object. Objects have properties and those properties have values. So an object is a key value pair. To create an object literal, we use **two curly brackets({})**.
...
[MDN Document][8]  /  [Stackoverflow][9]

+ Dot Notation
Use ' . ' followed by the key name if the key name is one word.

+ Square Bracket Notation(Computed properties) 
Use square bracket and a quote(['']) if the key name is a string.
<br>
