...
1. 무조건 for 문을 쓰는 게 아니라 filter(), includes(), map(), find()를 통해 쓸 수 있는지 확인해보자. 

2. 문제를 이해할 때, index와 value 영역을 나눠서 생각할 필요가 있다. 나는 계속 value 값에만 집착해서 문제를 풀려고 했으나, index로 접근하니 훨씬 간단하게 풀이가 되더라. 
