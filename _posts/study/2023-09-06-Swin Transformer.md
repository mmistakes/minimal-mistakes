...
## 1. Introduction
 - 컴퓨터 비전 모델링은 오랫동안 CNN(합성곱 신경망)에 의해 지배되었다.
CNN은 ImageNet 이미지 분류 챌린지에서 AlexNet의 혁신적인 성능으로 발전하기 시작했다.
CNN 아키텍처는 규모의 증가, 더 많은 연결, 더 정교한 형태의 합성곱을 통해 점점 더 강력해졌다.
자연어 처리(NLP) 분야의 네트워크 아키텍처 진화는 Transformer가 주류 아키텍처로 다른 길을 걸었다.
Transformer는 주목 기반으로 데이터의 장거리 의존성을 모델링하는 데 주목할 만하다.
연구자들은 Transformer의 컴퓨터 비전 적용 가능성을 탐구했으며, 이미지 분류와 시각-언어 모델링에서 유망한 결과를 보였다.
이 논문은 Transformer를 일반적인 컴퓨터 비전 백본으로 확장하고자 한다.
...
Swin Transformer는 계층적 표현을 구축하며, 깊은 Transformer 계층에서 인접 패치를 점진적으로 병합한다.
선형 계산 복잡성은 비중첩 윈도우 내에서 지역적으로 자기 주목(self-attention)을 계산함으로써 달성된다.
Swin Transformer는 다양한 비전 작업에 적합한 일반적인 백본으로 적합하다.
Swin Transformer의 핵심 설계 요소는 연속 자기 주목 계층 간의 윈도우 분할 이동이다.
...
본 연구는 여러 기본 시각 인식 작업에서 강력한 성능을 달성하며, 모델링 전환에 기여하기를 희망한다.
 ...
 - NLP 분야의 자기 주목(self-attention) 계층과 Transformer 아키텍처의 성공에 영감을 받아, 일부 연구는 인기 있는 ResNet의 공간 합성곱 계층 일부 또는 전체를 자기 주목 계층으로 대체한다.
이러한 연구에서는 각 픽셀의 지역 윈도우 내에서 자기 주목을 계산하여 최적화를 가속화한다.
...
슬라이딩 윈도우를 사용하는 대신, 연속 계층 간에 윈도우를 이동시키는 것을 제안한다. 이는 일반 하드웨어에서 더 효율적인 구현을 가능하게 한다.
 ...
 - 일부 연구는 표준 CNN 아키텍처를 자기 주목 계층 또는 Transformer로 보강한다.
자기 주목 계층은 백본 또는 헤드 네트워크에 장거리 의존성 또는 이질적 상호작용을 인코딩하는 능력을 제공함으로써 보완할 수 있다.
...
ViT는 잘 수행하기 위해 대규모 훈련 데이터셋(예: JFT-300M)이 필요하지만, DeiT는 ViT가 더 작은 ImageNet-1K 데이터셋을 사용하여도 효과적일 수 있도록 여러 훈련 전략을 도입한다.
...
이러한 패치 토큰에 수정된 자기 주목 계산을 갖는 여러 Transformer 블록(스윈 Transformer 블록)이 적용된다.
...
Swin Transformer 블록은 표준 다중 머리 자기 주목(MSA) 모듈을 이동된 윈도우를 기반으로 한 모듈로 대체함으로써 구축된다.
...
자기 주목 계산에서 상대 위치 편향을 포함하여 유사도를 계산한다.
상대 위치 편향을 포함한 모델은 절대 위치 임베딩을 사용하는 모델이나 편향 용어가 없는 모델보다 상당한 개선을 보인다.
학습된 상대 위치 편향은 사전 훈련에서 다른 윈도우 크기로 미세 조정을 위한 모델 초기화에도 사용될 수 있다.
...
정규 ImageNet-1K 훈련 설정은 대부분 [63]을 따르며, AdamW 최적화기를 사용하여 300 에폭 동안 코사인 감쇠 학습률 스케줄러와 20 에폭의 선형 웜업을 사용한다.
ImageNet-22K에서 사전 훈련 후 ImageNet-1K에서 미세 조정하는 설정도 포함한다.
정규 ImageNet-1K 훈련 결과, Swin Transformer는 유사한 복잡성을 가진 DeiT 아키텍처를 눈에 띄게 초과한다: Swin-T는 DeiT-S보다 +1.5% 높은 정확도를 달성한다.
ImageNet-22K 사전 훈련 결과, Swin-B는 ImageNet-1K에서 처음부터 훈련한 것보다 1.8%~1.9%의 이득을 가져온다.
...
시스템 수준 비교를 위해 개선된 HTC(HTC++)를 채택하며, ImageNet-22K 사전 훈련 모델을 초기화로 사용한다.
...
ImageNet-22K 사전 훈련을 받은 Swin-L 모델은 val 세트에서 53.5 mIoU를 달성하여, 더 큰 모델 크기를 가진 SETR보다 +3.2 mIoU 높은 이전 최고 모델을 초과한다.
...
다른 자기 주목 계산 방법의 실제 속도를 비교했으며, 순환 구현은 특히 더 깊은 단계에서 하드웨어 효율적이다.
제안된 이동된 윈도우 기반 자기 주목 계산은 Performer와 같은 가장 빠른 Transformer 아키텍처보다 약간 빠르며, ImageNet-1K에서 Swin-T를 사용한 Performer에 비해 +2.3% 더 높은 top-1 정확도를 달성한다.
...
Swin Transformer의 핵심 요소인 이동된 윈도우 기반 자기 주목은 비전 문제에서 효과적이고 효율적임이 입증되었으며, 자연어 처리에서의 사용을 탐구하는 것을 기대한다.