...
grep 커맨드 중 [y] 는 `grep` 프로세스 자신을 결과에 풀쳑하지 않는 것이다.  
6개의 **mingetty 프로세스**가 확인 되었고, 이는 **6개의 가상 콘솔 login: 프롬프트**에 대응된다.  
여기서 유저명을 입력해서 **패스워드 입력 프롬프트**가 표시될 때까지 진행한 뒤 다시 한번 **mingetty 프로세스들**을 확인해 보자.
...
출력된 결과를 보면 프로세스 수가 하나 줄었고, 이를 사라진 프로세스 id(1692)로 검색하면 **login 프로세스**로 표시되는 것을 볼 수 있다.
``` bash
# ps -ef | grep "169[2]"
