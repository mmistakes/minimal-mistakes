...
use_math: true
---
*자연 장면에서 임의 형태의 텍스트를 탐지하는 것은 매우 도전적인 작업입니다. 기존의 텍스트 탐지 방법들이 제한된 특징 표현을 기반으로 텍스트를 인식하는 것과 달리, 우리는 더 풍부한 특징을 융합하여 텍스트 탐지를 수행하는 새로운 프레임워크인 TextFuseNet을 제안합니다. 더 구체적으로, 우리는 문자-, 단어-, 글로벌 수준의 세 가지 특징 표현에서 텍스트를 인식하고, 강력한 임의 형태 텍스트 탐지를 달성하기 위한 새로운 텍스트 표현 융합 기술을 도입합니다. 실제로, 제안된 TextFuseNet은 임의 형태의 텍스트를 보다 적절하게 설명하고, 거짓 긍정을 억제하며, 더 정확한 탐지 결과를 생성할 수 있습니다. 우리의 프레임워크는 문자 수준 주석이 없는 데이터셋에 대해서도 약한 감독으로 훈련될 수 있습니다. 여러 데이터셋에서의 실험 결과, 제안된 TextFuseNet은 최고의 성능을 달성했습니다. 구체적으로, ICDAR2013에서 94.3%, ICDAR2015에서 92.1%, Total-Text에서 87.1%, CTW-1500에서 86.6%의 F-측정값을 달성했습니다.*
...
## 1 Introduction
 - 도입 및 동기: 컴퓨터 비전 커뮤니티 내에서 점점 관심이 커지는 장면 텍스트 탐지에 대해 강조하며, 다양한 모양, 복잡한 배경, 불규칙한 모양, 질감 간섭 등으로 인한 도전을 강조합니다. 기존 방법들을 문자 기반과 단어 기반 접근으로 나누고, 임의의 모양의 텍스트를 탐지하는 데 있어서의 한계점을 개요합니다.

TextFuseNet 프레임워크: TextFuseNet을 제안하여, 문자-, 단어-, 전역 수준 특징을 융합하여 임의의 모양의 텍스트를 탐지할 수 있도록 Mask R-CNN을 확장합니다. 다중 경로 특징 융합 아키텍처와 문자 수준 주석이 없는 경우에도 효과적인 학습을 가능하게 하는 약한 감독 학습 방식을 소개합니다.

기여도: 텍스트 탐지를 위해 특징을 융합하는 새로운 프레임워크인 TextFuseNet을 도입하고, 단어 수준 주석을 활용하는 약한 감독 학습 방식을 개발하며, 임의의 모양을 가진 텍스트가 포함된 여러 벤치마크에서 최고 성능을 달성했다는 세 가지 주요 기여를 나열합니다.

## 2 Related Work
 - 문자 기반 방법: 복잡한 문자 탐지기를 사용하여 문자 후보를 추출하고, 문자/비문자 분류기로 거짓 후보를 제거한 뒤, 남은 문자들을 단어로 그룹화합니다. 이 방법들은 설계가 복잡하고 여러 단계의 처리를 포함하여 시간이 많이 걸리고 최적이 아닙니다.
단어 기반 방법: 일반 객체 탐지 방법에서 영감을 받아 직접 단어를 탐지합니다. 이 방법들은 주로 수평 또는 다양한 방향의 텍스트에 적합합니다.
임의 형태 텍스트 탐지를 위한 인스턴스 세그멘테이션 기반 방법: 임의의 형태를 가진 텍스트를 탐지하기 위해 여러 방법이 제안되었습니다. 이러한 방법들은 텍스트/비텍스트 예측, 링크 예측, 텍스트 영역 위치 지정, 다양한 비율의 텍스트 인스턴스에 적응적으로 훈련을 조정하는 방법 등을 포함합니다.
본 연구의 차별점: 이전 연구와 비교하여, 다양한 수준의 특징을 분석하고 융합하여 텍스트 탐지 성능을 효과적으로 향상시키는 더 풍부한 융합 특징을 얻습니다.
