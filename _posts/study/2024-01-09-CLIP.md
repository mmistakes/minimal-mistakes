...
추가 데이터나 훈련 없이도 맞춤형 응용 프로그램으로 CLIP을 조정할 수 있는 능력.
...
알고리즘 결정, 훈련 데이터, 클래스 디자인이 AI 시스템 사용으로 인한 사회적 편향과 불평등을 증폭시킬 수 있음.
CLIP의 편향 분석:
...
모델은 사전 훈련 데이터에서 유명인 얼굴을 이름과 연관시키는 능력 테스트.
유명인 식별 테스트 결과:
...
텍스트-이미지 검색을 최적화하기 위한 CLIP의 사전 훈련 작업.
웹 감독 학습:
...
NLP에서의 과업 비특정적 웹 규모 사전 훈련의 성공을 다른 도메인으로 전환할 수 있는지 여부 조사.
...
CLIP 모델은 사전 훈련 중 다양한 작업을 수행하도록 학습.
