...
<img src="../../assets/images/2024-01-09-CLIP/Fig6.jpg" alt="Figure_6" style="zoom:56%;"/>
...
임의의 이미지 분류 작업 수행 가능.
제로샷 설정에서 이미지 검색이나 검색과 같은 광범위하게 적용 가능한 작업에서 상당한 잠재력 보여줌.
맞춤형 응용 프로그램으로의 쉬운 조정:

추가 데이터나 학습 없이도 맞춤형 응용 프로그램으로 CLIP을 조정할 수 있는 능력.
대규모 언어 모델과 유사하게, 상상하기 어려운 새로운 응용 프로그램이 가능함.
감시와 같은 하류 작업에서의 성능:

FairFace 벤치마크에서 성능 평가 및 편향 조사 수행.
감시 등 사회적 함의가 있는 분야에서의 유용성 논의.
다목적 사용 가능한 기능:

OCR, 동작 인식, 객체 분류, 지리적 위치 파악, 얼굴 감정 인식 등의 기능이 감시에 사용될 수 있음.
사회적 편향의 특성화 시도:

모델에 내재된 사회적 편향을 이해하기 위한 초기 노력.
특정 배치와 관련하여 모델 분석 필요.
테스트 및 평가의 필요성:

더 넓고, 더 맥락적이며, 더 강건한 테스트 체계 개발을 통해 일반 목적 컴퓨터 비전 모델에서의 편향 더 잘 특성화 필요.

### 7.1. Bias
편향의 기원:

알고리즘 결정, 학습 데이터, 클래스 디자인이 AI 시스템 사용으로 인한 사회적 편향과 불평등을 증폭시킬 수 있음.
CLIP의 편향 분석:

FairFace 데이터 세트에서 CLIP의 성능을 분석하여 초기 편향 조사 실시.
로지스틱 회귀 분류기와 제로샷 CLIP 모델의 성능 비교.
성별, 인종, 연령에 대한 모델의 성능:

성별 분류에서 모든 인종 카테고리에 대해 95% 이상의 성능.
인종과 성별에 따른 성능 차이 발견.
클래스 디자인의 영향:

클래스 디자인이 모델 성능과 원치 않는 편향 또는 행동을 결정하는 중요한 요인.
클래스 추가가 모델의 행동과 연령별 비하 피해 분포를 변화시킬 수 있음.
실험을 통한 편향 관찰:

고위직 직업에 대한 성별 편향, 낮은 임계값에서 성별에 따른 라벨 분포 차이 관찰.
의회 구성원 이미지에 대한 성별 예측 성능 조사.
모델 설계 결정의 중요성:

클래스 디자인, 임계값 설정 등 모델 구축의 각 단계에서의 설계 결정이 편향을 어떻게 나타나게 하는지에 큰 영향.
모델 및 AI 시스템 개발자의 결정이 모델 성능 및 편향 발현 방식에 중요한 영향을 미침.
연구의 한계 및 목적:

실험들은 포괄적이지 않으며 클래스 디자인과 기타 편향 원인에서 비롯된 잠재적 문제를 보여줌.
추가 조사를 촉발하기 위한 것임.

### 7.2. Surveillance
감시 작업과 관련된 모델 성능 특성화:

사회적 민감성이 큰 하류 작업인 감시와 관련하여 모델 성능을 평가.
CCTV 이미지에 대한 모델 테스트:

저해상도 CCTV 카메라 이미지에 대한 모델 성능 테스트.
VIRAT 데이터 세트와 Varadarajan & Odobez(2009)의 데이터 사용.
감시 이미지에 대한 분류 테스트:

515개의 감시 이미지에 대한 조잡한 분류 및 세밀한 분류 테스트.
주요 주제 식별(조잡한 분류)과 이미지 내 작은 특징의 존재/부재 식별(세밀한 분류) 요구.
분류 테스트 결과:

조잡한 분류에서 91.8%의 최고 정확도 달성, 하지만 '근접' 대답 포함 시 정확도 51.1%로 감소.
세밀한 탐지에서는 제로샷 모델 성능이 무작위에 가까움.
유명인 식별 성능 테스트:

'야생'의 유명인 식별을 위한 CelebA 데이터 세트 사용.
모델은 사전 학습 데이터에서 유명인 얼굴을 이름과 연관시키는 능력 테스트.
유명인 식별 테스트 결과:

100가능 클래스 중에서 59.2%의 최고 정확도, 클래스 크기를 1k로 늘리면 43.3%로 감소.
Google의 Celebrity Recognition과 같은 생산 수준 모델과 비교할 때 경쟁력 없음.
CLIP의 잠재적 사용성:

제로샷 능력으로 데이터가 적은 작업에 이점 제공.
얼굴 인식과 같이 수요가 많은 감시 작업에는 이미 대규모 데이터 세트 및 고성능 모델 존재.
감시 작업에 대한 CLIP의 한계:

객체 탐지나 의미론적 분할과 같은 일반적인 감시 관련 작업에는 설계되지 않음.
CLIP은 Detectron2와 같이 이러한 용도로 설계된 모델에 비해 제한된 사용 가치를 가짐.
특정 감시 사용 사례에 대한 가능성:

잘 맞춤화된 모델이나 데이터 세트가 존재하지 않는 맞춤형, 틈새 감시 사용 사례에 유용할 수 있음.
ZS CLIP는 몇 가지 감시 관련 작업에서 중요하지 않은 성능을 보임.

### 7.3. Future Work
초기 분석의 목적:

일반 목적의 컴퓨터 비전 모델이 제기하는 도전과제를 설명하고, 이들의 편향과 영향에 대한 통찰을 제공하기 위함.
연구 커뮤니티와의 협력:

이러한 모델의 능력, 단점, 편향에 대한 특성화에 대한 향후 연구를 동기 부여하기 위해 연구 커뮤니티와 협력할 의향.
커뮤니티 탐색의 중요성:

CLIP과 같은 모델의 능력을 더 잘 특성화하고 유망한 성능을 가진 응용 분야와 성능이 감소할 수 있는 분야를 식별하기 위한 커뮤니티 탐색 강조.
모델 사용의 유익성 증가 방안:

모델의 잠재적으로 유익한 하류 사용 식별.
민감한 과제 및 큰 사회적 이해 관계자 세트 드러내기.
모델의 편향을 더 잘 특성화.
시스템 평가를 위한 테스트 스위트 생성.
잠재적 실패 모드 및 추가 작업 영역 식별.
향후 연구 기여 계획:

이 분석을 통해 후속 연구에 대한 동기 부여 예제를 제공하고, 이와 관련된 연구에 기여할 계획.

## 8. Related Work
자연어 감독의 활용:

언어 모델과 분포 의미론 분야의 연구, 예를 들어 주제 모델, 단어/문장/단락 벡터, 언어 모델 등이 자연어를 감독의 원천으로 사용.
NLP 분야의 연구:

자연어 감독을 활용한 대화 기반 학습, 의미 파싱, ExpBERT 등의 연구.
컴퓨터 비전에서의 자연어 활용:

Ramanathan et al. (2013)의 연구로부터 시작하여 이미지 검색 및 객체 분류에 자연어 설명 활용.
자연어 감독을 활용한 다양한 연구:

의미론적 분할, 새의 세밀한 시각 분류, ShapeWorld 데이터 세트에 대한 연구 등.
강화 학습과 자연어의 결합:

강화 학습 환경과 자연어를 결합한 연구들이 제로샷 작업을 체계적으로 수행하는 능력을 보여줌.
텍스트-이미지 검색 분야의 연구:

이미지-텍스트 검색에 대한 초기 연구부터 멀티모달 임베딩 공간 학습으로의 이동.
자연어 감독을 활용한 기타 분야의 연구:

비디오와 서술적 텍스트, 밀집된 구어체 자연어 감독을 사용한 연구.
CLIP의 사전 학습 작업:

텍스트-이미지 검색을 최적화하기 위한 CLIP의 사전 학습 작업.
웹 감독 학습:

이미지 검색 엔진을 사용하여 이미지 데이터 세트를 구축하는 웹 감독 학습 연구.
비전과 언어의 공동 모델 학습:

시각적 질문 응답, 시각적 상식 추론, 다중모달 함축 등을 해결하기 위해 비전과 언어를 연결하는 최근의 연구 활동.

## 9. Conclusion
LP의 사전 학습 전략 탐구:

NLP에서의 과업 비특정적 웹 규모 사전 학습의 성공을 다른 도메인으로 전환할 수 있는지 여부 조사.
컴퓨터 비전 분야에서 유사한 행동 발견:

이러한 접근 방식을 채택하면 컴퓨터 비전 분야에서 유사한 행동이 나타남.
사회적 함의 논의:

이 연구 분야의 사회적 함의에 대한 논의.
CLIP 모델의 학습 능력:

CLIP 모델은 사전 학습 중 다양한 작업을 수행하도록 학습.
자연어 프롬프트를 통한 제로샷 전송 가능:

자연어 프롬프트를 통해 많은 기존 데이터 세트에 대한 제로샷 전송 활용 가능.
성능과 경쟁력에 대한 평가:

충분한 규모에서, 이 접근법의 성능은 과업 특정 감독 모델과 경쟁력이 있음.
여전히 많은 개선의 여지 존재.