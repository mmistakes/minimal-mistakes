...
### 3.2. Model Design
 ...
 - **“Swiss Army Knife” Model: InternVL.**
   ...
   - (2) *For contrastive tasks*, 비전 인코더 또는 InternViT와 QLLaMA의 결합을 사용하여 시각 특징을 인코딩한다.
   - (3) *For generative tasks*, QLLaMA는 확장된 매개변수 덕분에 이미지 캡셔닝 능력이 있다.
   - (4) *For multi-modal dialogue*, InternVL을 LLM과 연결하는 시각 구성요소로 활용한다.
