...
매출에 연관해서 가설을 잡아야 할 것 같아 첫 번째 가설을 막연하게 정해버렸다. **매출은 연도별로 상승했을 것이다.** 좀 뜬금없는 가설을 설정해버렸다. ㅎ

또한, 막상 가설 검증을 하고 나니 그래서 어쩌란 거지? 이런 생각이 들었다.

가설 검증 후 마케팅 전략을 위한 인사이트 도출하기에도 적합한 분석 방향이 아니었다.

데이터 그룹을 나눌 때 신규 사용자/기존 사용자와 같은 고객 데이터 중심으로 분석 방향을 정했으면 비즈니스적으로 의미 있는 인사이트 도출에 더 나아갔을 것 같다는 생각이 든다.