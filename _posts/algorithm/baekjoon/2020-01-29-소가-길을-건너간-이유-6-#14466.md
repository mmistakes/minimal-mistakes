...
---
처음에 문제를 이해하는 게 어려웠다.  
문제 설명을 먼저 하자면, 서로 다른 위치에 있는 소들이 주어진 길(R)로만 만날 수 있는 쌍을 구하는 문제였다.  
...
시간을 단축하기 위해서 시작 소 -> 모든 위치 dfs를 했고,  시작 소가 방문하지 못한 위치에 다른 소가 존재한 경우를 count 했더니 시간 초과를 해결할 수 있었다.  
