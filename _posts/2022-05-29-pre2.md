...
\1) 한 개 단위로 자르는 경우
 ...
 전체 문자열 내에 중복여부는, 반복문 내에서 

 <>기호 안에 표시된 문자열을 앞에서부터 

 차례로, former, latter에 넣어 판단한다.

​    

 중복되는 문자열이 있는 경우, 전체 문자열을 자르는

 단위에 만큼씩 길이를 줄일 수 있다.

 그런데, 중복된 횟수 나타내는 숫자를 십진법으로 

 표시해야 하므로, 길이가 늘어나게 된다.
...
∴문자열 "aabbaccc“는 한개 단위로 잘라 “2a2ba3c”로 나타낼 때,  가장 짧게 표현 가능하다
