...
- <mark style='background-color: #f1f8ff'>운영 장비에는 절대 create, create-drop, update 절대 사용하면 안된다.</mark> 📌
- **개발 초기 단계**는 **create** 또는 **update**만 사용.
- **테스트 서버**는 **update** 또는 **validate** 사용.
- **스테이징**과 **운영 서버**는 **validate** 또는 **none**.
- <mark style='background-color: #f1f8ff'>update는 DB에 alter를 날리기 때문에 DB에 락이 걸릴 수 있기에 사용을 지양해야 한다.</mark> 📌
