...
이 기능은 특정 연결 활동을 추적할 수 있도록 도와줍니다. ProxySQL를 사용하는 경우 DB 접근을 위한 단일 지점에 해당하는 경우가 대다수 이기 때문에 ProxySQL 을 통한 데이터베이스 접근에 대한 기록을 남겨두는 것은 중요한 부분입니다. 이 기능을 활성화하려면, mysql-auditlog_filename 변수를 구성하여 로그가 기록될 파일을 지정해야 합니다. 이 변수의 기본값은 빈 문자열이며, 기본적으로는 로그 기록이 활성화되지 않습니다.
...
---
시각화 도구인 kibana를 통해 수집한 로그를 확인합니다. 이를위해 로그스태시 URL 주소에 접근합니다. 

```
http://kibana주소:5601
```

그리고 elasticsearch 구성시 생성한 elastic 계정의 패스워드로 로그인합니다.
아래는 proxySQL의 쿼리로그의 시각화 예시입니다. 

좌측 햄버거 아이콘을 클릭 후 Discover 에 들어갑니다.

![Dicover](https://github.com/user-attachments/assets/e961e937-bfdd-49c3-93f4-f8a8f382e2c1 "Discover 진입 화면")
...
Create data view 상세화면에 진입하면 data view 명칭과 인덱스 패턴을 설정합니다. 저는 data view 명칭은 mysql-proxysql-query-logs 라고 기재하였습니다.(이미 만들어 놓았기 때문에 빨간색 글씨로 경고 문구가 나왔습니다만 최초 생성한다면 해당 문구는 없습니다.) 그리고 인덱스 패턴은 mysql-proxysql-query-logs-* 로 설정하였습니다. 우측에 해당 인덱스 패턴과 매칭되는 소스들을 확인할 수 있습니다. 그리고 Save data view to Kibana 를 눌러 설정을 반영합니다.
...
그리고 우측 행의 좌측의 확대 아이콘(Toggle dialog with details)을 클릭하면 아래 그림처럼 해당 도큐먼트에 설정된 모든 필드를 확인할 수 있습니다. 
