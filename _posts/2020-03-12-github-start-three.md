...
깃허브 블로그를 이용하는 사람들 중 많은 사람들이 본인의 `github` 프로젝트를 포스팅하거나, 공부한 자료 등을 정리하는 용도로 블로그를 운영하는 것으로 알고 있다. 그 과정에서 프로그래밍 코드나 알고리즘을 정리하기도 할 텐데, 포스트에 코드를 작성하고 싶을 때는 키보드에서 숫자 1 옆에 위치한 **backquote** 문자 (shift 눌렀을 때 `~`로 표시되는 문자)를 이용하면 된다.
