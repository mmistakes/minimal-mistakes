...
- 직접 확인한 결과: 반환값이 필요한 함수 마지막 줄에 세미콜론을 붙이면 `()`로 처리돼 의도한 타입과 맞지 않는 흐름이 생겼다.
...
- 의견: 초급자 기준으로는 변수, 타입, 제어흐름, 함수를 따로 외우기보다 하나의 `main.rs`에서 번갈아 실행하며 보는 편이 훨씬 이해가 빠르다.
- 의견: 타입 표 전체를 한 번에 암기하기보다 `i32`, `bool`, `&str`, `String` 같은 자주 쓰는 타입부터 익히는 편이 실용적이다.
