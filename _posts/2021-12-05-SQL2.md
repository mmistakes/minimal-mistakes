...
```sql
(select a.team_id, date(a.crt_date) as team_crt_date, b.clan_id from 
 테이블1 a left join 테이블2 b  
 on a.team_id = b.team_id where a.team_id >= 조건1 and a.team_id < 조건2 and a.status = 'OK' )

union

(select a.team_id, date(a.crt_date) as team_crt_date, b.clan_id 
 from 테이블1 a right join 테이블2 b  
 on a.team_id = b.team_id 
 where a.team_id >= 조건1 and a.team_id < 조건2 
 and a.status = 'OK' ) 
...
- 알고 싶은 것 : 패키지 A 또는 B 구매자의 team_id를 알아내고, 몇 명인지 바로 알기 위해 rownum을 붙이기 
...
```sql
#1 돌아감 but 중복 있음 - 삽질1
select distinct a.team_id, b.team_name,@rownum := @rownum +1 as rownum from
...
#2 아예 안 돌아감 - 삽질2
select @rownum := @rownum +1 as rownum, distinct a.team_id, b.team_name, b.hub_name from
...
#3 정답1
select c.rownum, c.team_id, b.team_name
from
...
'원스토어 패키지 B')
group by a.team_id
order by a.team_id) c, 팀테이블 b
where c.team_id = b.team_id;
 
 
 
#4 정답2
select @rownum := @rownum +1 as rownum, a.team_id, b.team_name from
(select distinct team_id from 구매테이블 where crt_date >= '2021-02-10 12:00:00'
and
product_id in ('ios 패키지 A',
'ios 패키지 B',
'안드로이드 패키지 A',
...
#5 프로덕트 테이블 이용한 정답
select @rownum := @rownum +1 as rownum, x.team_id, y.team_name from
(select distinct a.team_id from 구매테이블 a, 프로덕트테이블 b 
