...
class Solution 
{
    ...
    {                
        ...
        {    
            ...
            //(upper가 0이면, present_idx가 1이고, 이것은 최상위 rootnode를 의미하므로, upper_loc_value값은 val이된다.
            //만약 0이 아니라면, upper_loc_value값은 현재 노드의 값을 집어넣어야 한다.)
            upper_loc_value = (upper!=0) ? nodes_group[upper-1].vals[nodes_group[upper-1].loc_in_group] : val;
