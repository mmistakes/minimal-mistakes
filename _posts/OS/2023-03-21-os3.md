...
여러 개의 작업을 동시에 main memory에 적재하고, 적재된 작업들이 CPU를 번갈아 사용하게 함(multiplexing)
 ...
 - 단일 작업이 CPU와 I/O장치를 모든 시간에 사용할 수 없다.

**CPU efficiency 증가**

어떤 작업이 I/O 수행동안 또는 사건을 기다리는 동안 CPU를 계속하여 사용할 수 없을 때에 메모리에 적재된 다른 작업이 CPU를 사용하게 함
...
Interrupt가 발생하면 kernel mode로 전환됨
