...
- 모든 노드에 대한 고유한 호스트 이름, 고유한 MAC 주소, 고유한 product_uuid
    - 1.	MAC 주소 확인방법 : ifconfig –a , ip link
    - 2.	Product_uuid 확인방법 : sudo cat /sys/class/dmi/id/product_uuid
-	스왑 메모리 비활성화
    - 1.	Pod를 할당하고 제어하는 kubelet은 스왑 상황을 처리하도록 설계되지 않았음.
    - 2.	이유는 kubernetes에서 가장 기본이 되는 Pod의 컨셉 자체가 필요한 리소스 만큼만 호스트 자원에서 할당 받아 사용한다는 구조이기 때문이다.
    - 3.	따라서 kubernetes 개발팀은 메모리 스왑을 고려하지 않고 설계했기 때문에 클러스터 노드로 사용할 서버 머신들은 모두 스왑 메모리를 비활성화 해줘야 한다.
    - 4.	스왑 메모리를 비활성화 하기 위해 아래 명령어를 사용한다.
    - 5.	swapoff -a 와 sed -i '2s/^/#/' /etc/fstab


##	각 노드별 포트중첩제거
###	Master node에서 필요한 필수포트
