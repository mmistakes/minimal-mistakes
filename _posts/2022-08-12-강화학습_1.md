...
State <i>S<sub>t</sub></i>  가 Markov라는 것은 <i>P [ S<sub>t+1</sub> | S<sub>t</sub> ]_ = _P [ S<sub>t+1</sub> | S<sub>1</sub>, S<sub>2</sub> , ... , S<sub>t</sub> ]_</i> 와 동치이고, 이것을 Markov Property라 한다.
...
<center><i>P<sub>ss'</sub> = P [ S<sub>t+1</sub> = s' | S<sub>t</sub> = s ]</i></center>
...
여기서 <i>P<sub>n,1</sub></i> 는 state <i>1</i>에서 <i>n</i>으로 전이될 확률이다. 위의 행렬의 원소의 합은 <i>1</i>이다. 
