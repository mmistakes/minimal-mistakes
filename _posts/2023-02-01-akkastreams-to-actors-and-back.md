...
together. The reader is encouraged to study the example code on GitHub (see references) to get the full context and to
experiment with the examples. In real world scenarios using stream elements that originate from actors or enter an actor
...
Let's consider the common scenario subscribing to a Kafka topic and processing the messages. Assume that the processing
of each message involves actor based processing, i.e. not just processing in Akka flows within an Akka stream. This is
actually a not so simple thing to accomplish depending on your requirements.

There are basically two standard approaches both of which are described in the Akka documentation.
...
2. Use the ask pattern in an Akka Flow. This can be done in plain vanilla style or with the ActorFlow.ask flow which is
   basically just a small convenience utility.
...
different ways. See the code snippet where an Akka source is connected and run with the Alpakka committer sink. <br>
...
(see the example application on GitHub accompanying this article). <br>
...
The remaining requirements 2–4 are more problematic. <br>
The ask pattern is used with a timeout that while suitable for many scenarios, sometimes it may not make sense. A
...
separately and explicitly. You therefore face the risk of not cleaning up resources and similar. It should be noted
though that it is possible to e.g. handle the notification of the target actor outside of the stream itself by
...
messages without emitting. This is a perfectly viable usage in the example use case with Kafka message processing: Let's
assume that some messages cannot be processed immediately and you wish to store them for later processing, for example
...
The pseudo code above expresses some aspects of the use case sketched to show that there can be multiple emitted 
...
To setup a stream according to the diagram above we will have the target actor process StreamToActorMessage[T] messages 
...
Akka streams toolbox. In short the stage actor, although not being an actor, imitates some of the actor functionality 
...
and the other way is mediated via the stage actor and the receive used when it was first obtained. 
...
target actor. 
The StreamToActorMessage trait contains more or less the same type of messages used in the sink based approach.
...
### Putting the stage actor based approach to use
...
Remember that the stage actor based approach completely decouples the rate of elements consumed by the custom flow from
...
stageActorReceive defines the stage actor behavior. Without going into details the code should give a feeling of the 
...
On the other side, in the GraphStage handlers api we can see the corresponding for the streams interaction. 
...
the target actor and in an enterprise application setting this will many time already be present. The following is a 
...
since not all flow elements will carry the pass-through on. Note that the term pass-through now has a slightly diffferent 
meaning. It just means the the pass-through will accompany one message following the 
...
def onElementOut(elemOut: Any): Unit = {
      ...
      passThroughMessageMapping.remove(elem.id)
