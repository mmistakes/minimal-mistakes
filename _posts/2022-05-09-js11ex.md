...
> When I destructure an object, the variable I use should be the same as the key or property of the object.

```js
const newStudent = {...student}
const {name, age, skills} = newStudent;
let {frontEnd, backEnd, dataBase, dataScience} = skills;
Object.assign(skills, {skill: 'Bootstrap', level: 8});
backEnd.push({skill: 'Express', level: 9});
dataBase.push({skill: 'SQL', level: 8});
dataScience.push('SQL');
```
Then I decided to stop using `push()` and instead use `Object.assign()` at its place. 
