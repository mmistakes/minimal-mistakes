...
추가로, 위 그림에서 상위 `2 bits`는 `11110` 신호와 함께 전송되는 것을 확인할 수 있는데, 이는 `10 bits` 주소 값을 갖는 `Slave`에 접근하기 위해 고정적으로 사용되는 값이다. **따라서**, `7 bits` 주소 값을 갖는 `Slave` 주소는 `11110`으로 시작될 수 없으며, 하나의 `Master`에서 한 번에 제어할 수 있는 `7 bits` 주소의 `Slave`는 128 개가 아닌, **124 개**가 된다.
