...
3. 로그 우도 함수 \\(\log L(\theta; \mathbf{X})\\)로 변환:
...
5. 경사하강법으로 가능도를 최적화할 때 미분 연산을 사용하게 되는데, 로그가능도를 사용하면 연산량을 \\(\mathcal{O}(n^2)\\)에서 \\(\mathcal{O}(n)\\)으로 줄여줌
6. 대게의 손실함수의 경우 경사하강법을 사용하므로 음의 로그가능도(negative log-likelihood)를 최적화하게 됨
