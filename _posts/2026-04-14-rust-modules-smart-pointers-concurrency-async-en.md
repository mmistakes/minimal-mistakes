...
- Opinion: beginners usually understand `Rc` vs `Arc` and `RefCell` vs `Mutex` faster when they are first framed by role, such as single-thread sharing, runtime borrow checking, or multi-thread shared state.
- Opinion: async should be introduced as a separate execution model for waiting-heavy work, not as a simple replacement for threads.
