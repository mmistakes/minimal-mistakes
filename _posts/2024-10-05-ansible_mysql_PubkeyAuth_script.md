...
작업대상은 hosts 에 있는 서버 4대입니다. ansible_user 에 의해 root 로 접속을 하고 ansible_become 변수가 true 이므로 모든 명령어들이 sudo 로 실행됩니다.
...
스크립트 수행 이후 ssh 접속을 해보면 아래와 같이 패스워드 인증없이 정상적으로 연결되는 것을 확인할 수 있습니다.👍 아래 테스트는 mon-server1 에서만 수행한 결과만 보여드렸지만 실제로 작업 대상인 모든 서버에서 패스워드 인증 없이 SSH 연결이 가능합니다.
