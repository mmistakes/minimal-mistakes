...
![image](https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRVfMylYafjmeloDcIw5hspAvY_uJuLmYFKJQ&usqp=CAU)
...
우선 좋은 알고리즘의 가장 필수적인 선행 조건은 <span style="color:red; font-weight:bold;">'문제를 해결할 수 있는가'</span>의 여부일 것이다. 문제를 제대로 해결할 수 없는 알고리즘을 좋은 알고리즘인가 논하는 것은 어불성설에 지나지 않는다.



위의 선행 조건이 만족되었다면 이제 해당 알고리즘이 좋은 알고리즘인지 판별할 자격이 생긴다. 알고리즘의 성능은 <span style="color:red; font-weight:bold;">'문제를 얼마나 더 빨리, 더 적은 노력과 소모값으로 해결했는지'</span>에 따라 갈릴 것이다.



즉, 좋은 알고리즘의 결정조건은 <span style="color:red; font-weight:bold;">'해당 알고리좀의 문제 해결에 대한 효율성'</span>에 따라 결정된다고 할 수 있다.
...
앞서 알고리즘이 얼마나 효율적인지에 따라 좋은 알고리즘의 여부가 결정된다고 하였는데 컴퓨터 알고리즘에서 좋은 알고리즘의 판별 여부는 <span style="color:red; font-weight:bold;">시간과 공간</span>이다. 차후의 다른 포스팅에서 이에 대해 자세하게 설명할 예정이다.
