...
- 직접 확인한 결과: 2026-04-21 Windows PowerShell에서 Docker CLI와 Docker Engine이 모두 응답했고, 공식 예시 흐름에 가까운 컨테이너 실행, 조회, HTTP 접근, 중지 흐름을 확인했다. 로컬 포트 충돌을 피하려고 host port는 `8080` 대신 `18080`을 사용했다.
...
- 실행 결과 요약: `docker version`은 Client/Server 모두 `29.4.0`, Server OS는 `linux/amd64`로 출력됐다. `docker/welcome-to-docker:latest` 컨테이너는 `Up` 상태로 조회됐고, `http://localhost:18080` 요청은 HTTP `200`을 반환했다. `docker stop` 뒤 테스트 컨테이너는 제거됐다.
- storage 확인: `codex-volume-test` volume에 `volume-ok` 파일을 쓴 뒤 다른 컨테이너에서 같은 값을 읽었다. host bind mount로 연결한 임시 디렉터리의 `host.txt`도 컨테이너 안에서 `bind-ok`로 읽혔다.
...
2026-04-21 직접 재현은 Docker Desktop의 Linux container 환경에서 수행했다. native Windows container, rootless mode, user namespace isolation, 다른 운영체제의 Docker Desktop 파일 공유 동작은 확인하지 않았다. 실행 예시는 로컬 포트 충돌을 피하려고 host port `18080`을 사용했으므로, 공식 문서 예시의 `8080`과 포트 번호만 다르다.
