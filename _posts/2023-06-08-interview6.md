...
- 생성된 렌더 트리는 정해진 순서대로 화면에 표시되는데, UI 백엔드에서 렌더 트리의 각 노드를 가로지으며 형상을 만는는 그리기 과정이 진행.
...
- HTTP Protocol의 인프라를 그대로 사용하기 때문에 REST API 사용을 위한 별도의 인프라 구축을 안해도 된다.
- HTTP Protocol 표준에 따르는 모든 플랫폼에서 사용 가능
...
RESTful이란 REST의 원리를 따르는 시스템을 의미.<br>
...
> API(Application Programming Interface) : 한 프로그램에서 다른 프로그램으로 데이터를 주고 받기 위한 방법(Ex. 메뉴판)