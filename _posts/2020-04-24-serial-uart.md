...
위 그림에서 전압 레벨을 맞추기 위한 그라운드 (GND) 라인을 제외하면, UART 통신에서는 별도의 클럭 없이 **TX**와 **RX** 두 개의 라인만을 이용한다 (**e.g.** `A`에서 `B`로 데이터를 송신하는 경우, `A.TX`핀과 `B.RX`핀 사이의 하나의 라인만을 이용). 즉, UART 통신을 이용하는 경우 하드웨어 연결이 단순하다.
...
UART 인터페이스에서는 별도의 클럭을 사용하지 않기 때문에, **Start bit**와 **Stop bit**를 이용해 통신의 시작과 끝을 구분한다.
...
위 그림은 **TTL** level (logical `HIGH`: **3.3 or 5V**)에서의 UART 데이터 스트림 예시를 보여준다. 기본적으로 통신 전 IDLE 상태에서는 logical `HIGH` 전압을 유지하고, `LOW` (**Start bit**) 로 바뀌는 시점부터 데이터 전송이 시작된다. 데이터는 **LSB**에서 **MSB** 순서로 전달하며, 데이터 전송이 끝나면 **Stop bit** 이후 `HIGH` 상태를 유지한다.
