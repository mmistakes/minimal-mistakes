...
### 5-3. flask 설치 (1.1.2)
### 5-4. 의존성 기록 (requirements.txt 작성)
...
$ flask run
```
### 5-7. FLASK_APP & FLASK_ENV & FLASK_DEBUG 이해
...
```
    ...
    1) $ flask run
        1-1) flask run할떄 create_app이란게 있으면 무조건실행시켜준다. 그때 return 받은 app을 flask run에서 이용한다.

    2)  $ __init__.py
        2-1) __init__.py가 해당 디렉토리안에 있다면, 해당 디렉토리는 모듈화가 된다. 그러면 flask run할때 __init__.py안에 있는게 실행된다.
...
```
    ...
    1) 원래 환경변수 설정을 set FLASK_APP=googlekaap/app.py:app 이렇게 안줘도 된다. 
    2) create_app을 자동으로 flask가 실행시켜 줄것이며, 
    3) 우리 모듈명은 __init__.py을 통해서 googlekaap디렉토리가 모듈화가 되었기 때문에 파이썬파일로 인식하게 된다.
    4) 그래서 set FLASK_APP=googlekaap 만 줘도 된다. 
...
def create_app():
    ...
    return app
```
    1) decorator에 URL로 <> 꺽새 넣으면 def 정의시 해당 parameter르 인자로 넣어줘야 한다.
    2)  http://127.0.0.1:5000/text/name/hello 여기로 접속해서 하나씩 확인해보자.
    3) 기본적으로 <>안에서 들어오는건 str임을 확인할수 있다.
