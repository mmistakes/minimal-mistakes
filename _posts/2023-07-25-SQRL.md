...
Let $X$ be some random variable for a datapoint and $\phi(X)$ be some statistic of $X$.
Then, if
...
for some $\delta$ and $a$, then we say that the rule $a \leq \phi(X) \leq b$ is a $1 - \delta$ quantile rule.
...
We specify the characteristics of the rules to be generated using a *rule schema*, as shown in the figure:
...
First, we generate *abstract rules* that structurally conform to the rule schema, as so:
