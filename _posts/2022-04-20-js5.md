...
### 5. indexOf
It **checks if an item exists in an array**. It returns the index if the value exists, and -1 if the value doesn't exist.
```js
...
### 11. [slice][3]
It cuts out a multiple items in range and returns a portion of array. *It takes two parameters: starting and ending position.*
`starting position <= slice < ending position(not included)`
If I only put one parameter, it would return array elements from that parameter through the end of an array.
```js
slice()
slice(start)
slice(start, end)
