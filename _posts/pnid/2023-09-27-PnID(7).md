...
<body>
  ...
  <script>
    async function runModel(imageUrl, selectedBackend) {
      ...
      image.onload = async function() {
        ...
        session.loadModel(modelFilepath).then(() => {
          ...
          const inputTensor = preprocessImage(imageElement); // 이미지 전처리 함수
    ...
    function preprocessImage(ctx, canvas) {
      ...
      const channels = 4; // RGB 이미지이므로 3개의 채널
      ...
      // const tensor = new Tensor(new Float32Array(imageData), 'float32', [1, 3, 416, 416]);
      return tensor;
    }
