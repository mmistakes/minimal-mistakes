...
## 4. 심볼 인식 모델 구축 과정
### 데이터 수집 및 준비
딥러닝 모델 학습을 위해서 실제 건설 프로젝트의 설계 도면을 수집했습니다. 수집된 설계 도면은 건설 사업의 규모에 따라 수량이 다양했으며, 대체로 200장에서 600장 사이의 양이었습니다. 본 연구를 위해 특정 건설 사업을 선정하였고 설계 도면을 약 600장 준비했습니다.<br>
수집한 설계 도면의 형태는 스캔된 PDF 형태였기 때문에 딥러닝 모델 학습을 위해 이미지 파일로 변환했습니다. 우선, 도면 데이터를 500dpi(Dots per inch)로 변환했고, 4678px * 3307px으로 리사이징되었습니다. Dpi는 해상도를 뜻하는데 낮은 dpi는 도면 이미지가 식별하기 어렵게되고, 너무 높은 dpi를 설정하면 도면 이미지의 사이즈가 너무 커지게 되어 dpi를 조정하면서 적당한 이미지의 해상도와 크기를 선정했습니다.<br>
딥러닝 모델을 학습시키기 위해서는 인식하고자 하는 심볼의 영역과 해당 심볼의 클래스명이 태깅된 레이블링 데이터가 필요합니다. 이를 위해 Semi-Auto Labeling 도구를 사용하여 레이블링 작업을 수행했습니다. 이 작업은 데이터셋의 품질을 향상시키는 데 중요한 역할을 하기 때문에 여러 차례 크로스체크를하여 준비했고, 총 30장의 도면에 레이블링 작업을 완수했습니다.

### 데이터 전처리
레이블링 된 데이터를 딥러닝 모델에 학습하기 위해서 추가적으로 몇가지 더 작업할 것이 있습니다. 첫번째로 학습에 사용될 데이터셋의 수량이 절대적으로 부족했고, 다양한 케이스 확보를 할 필요가 있었습니다. 두번째로 모델의 인풋 레이어에 입력되기위해 이미지를 조정할 필요가 있었습니다.

첫번째를 해결하기 위해서 Data Augmentation을 수행하였습니다. 도면 데이터를 아래 코드와 같이 roate(90, 180, 270), flip(상하, 좌우)로 증강시켰습니다.
<details>
...
<br>
두번째 이미지 조정하기 위해 도면 이미지를 패치 단위로 잘랐습니다. 원본 도면 이미지의 사이즈는 4678px * 3307px 이므로 모델에 입력될 때 사이즈가 줄어들며 심볼이 너무 작아져 인식하기가 어려웠기 때문에 Mask R-CNN 논문에서 권장하는 1024px * 1024px 사이즈로 패치 사이즈로 정했습니다. 이미지 패치는 단순히 격자로 자르게 되면 잘리는 부분이 정확히 학습되기 어렵기 때문에 sliding window 방식을 활용하였습니다.<br>
겹치는 부분(overlap)을 70%를 설정하여 sliding window를 통해 도면 한 장당 116장의 패치 이미지로 추출했습니다.<br>
마지막으로 레이블의 포맷을 coco 데이터셋 레이블 포맷을 변환하였고, train과 validation의 비율을 각각 8:2로 구분하였다.
