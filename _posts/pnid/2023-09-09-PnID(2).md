...
## 4. 심볼 인식 모델 구축 과정
### 데이터 수집 및 준비
회사가 수행했던 건설 프로젝트의 실제 설계 도면을 수집했습니다. 건설 사업의 규모에 따라 설계 도면의 수량이 다양했으며, 대체로 200장에서 600장 사이의 도면을 수집했습니다. 특히, 심볼 인식 연구를 위해 선정한 건설 사업의 설계 도면은 약 600장 정도였습니다. 
수집한 설계 도면은 스캔된 PDF 형식으로 제공 받았습니다. 이를 딥러닝 모델 학습을 위해 적절한 형식으로 변환해야 했습니다. 우선, 도면 이미지를 500dpi로 변환했고, 3000px * 4000px 크기로 조정되었습니다. 
딥러닝 모델을 학습시키기 위해서는 인식하고자 하는 심볼의 영역과 해당 심볼의 클래스명이 태깅된 레이블링 데이터가 필요합니다. 이를 위해 Semi-Auto Labeling 도구를 사용하여 레이블링 작업을 수행했습니다. 이 작업은 데이터셋의 품질을 향상시키는 데 중요한 역할을 했습니다.
레이블링 작업을 통해 학습 도면 데이터셋을 30장까지 준비했고, 8:2 비율로 각각 training dataset과 validation dataset으로 구분했습니다.
