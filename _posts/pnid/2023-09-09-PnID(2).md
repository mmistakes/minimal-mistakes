...
## 4. 심볼 인식 모델 구축 과정
### 데이터 수집및 준비
회사에서 실제로 수행했던 건설 사업의 설계 도면을 수집하였습니다. 건설 사업의 규모에 따라 설계 도면의 개수는 달라지게 되는데요. 사업마다 수집했던 설계 도면은 약 200~600장 정도였고, 심볼 인식 연구를 위해 선정했던 건설 사업의 설계 도면은 약 600장 규모였습니다.
설계 도면은 최초 스캔된 PDF형태로 제공 받았고 이를 500dpi로 이미지 변환하였더니 3000px * 4000px 사이즈로 변환되었습니다.
딥러닝 모델을 학습하기 위해서는 인식하고자 하는 심볼의 영역과 클래스명이 태깅된 레이블링 데이터가 필요하여 Semi-Auto Labeling 도구를 활용하여 작업하였습니다.
레이블링 작업을 통해 학습 도면 데이터셋을 30장 준비하였습니다.
