...
복사한 커맨드를 터미널에 붙여넣기 하고 실행을 하면 된다. (터미널은 **cmd + space**로 Spotlight를 켜서 실행하면 된다.) 그러면 맥 비밀번호를 입력하라는 창이 뜨는데, 비밀번호를 입력 후 엔터를 눌러준다. 그러면 뭔가가 설치되는 듯한 느낌이 들고, 그 뒤에 
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTUwMjIxMDM3NiwxNTA1Mjg2NTU2XX0=
-->