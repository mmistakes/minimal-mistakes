...
재시작을 하면 다이아몬드 아이콘부터 몇 개의 아이콘들이 정상적으로 보이는 지를 물어보는 질문이 나온다. 보이는 대로 답을 해주면 그 다음부터는 Prompt style을 지정할 수 있다. 원하는 대로 입맛에 맞게 설정을 해준 후, 마지막에 **(1) Verbose (Recommended)**를 선택해주고, **(1) Verbose (Recommended)**
(작성중)
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjAxMzcyMTA0MiwtMTQzMzMyNzM1LC0xNT
