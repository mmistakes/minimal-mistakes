...
여기에서 **Theme**을 보면 여러가지가 있는데, **Defaul**t를 누르면 상단바랑 터미널 창의 색이 통일되면서 미니멀한 느낌을 주는게 마음에 든다. 
...
다음으로는 Profiles에서 Text를 보면, 폰트를 지정할 수 있다. 나는 여러가지를 해본 결과 Courier New가 가장 마음에 들어서 이걸로 해놨다. 한 번씩 글씨체를 바꿔주는 것도 코딩에 리프레시가 될 것 같다. 글씨 크기도 나는 조금 키웠고 자간, 행간을 1씩 키웠다.
...
마지막으로 상태창을 만들어줄건데, Profiles에서 Session을 보면, 맨 마지막에 Status bar enabled라는 칸이 있다. 이걸 체크해주고, **Configure Status Bar**

(작성중)
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTk4MjY5MzcxNCwtMTQzMzMyNzM1LC0xNT
