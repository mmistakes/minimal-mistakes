...
#### ***End Note***
***
- 메모리 누수를 막기 위해, 생성자 안에서 자원을 획득하고 소멸자에서 그것을 해제하는 RAII 패턴을 사용 합시다. 
- 이미 선언된 포인터를 RAII 방식으로 사용하려면 스마트 포인터를 사용하도록 합시다.
- 스마트 포인터 중 하나인 std::unique_ptr는 동일 포인터에 대한 소유권을 동시에 여러개의 변수가 가질 수 없고, 소유권을 가진 변수가
하나도 없을 경우 자원을 해제합니다.
