...
<task>
Given the RTL chunk for the top module:
...
3. Create a checklist table with:
   ...
   • Status = “Pending”
4. Ask the user to provide the RTL chunk for one selected submodule.
</task>

<input>
- RTL chunk for current module (initially top module)
- (Optional) Existing checklist (none for first run)
</input>

<constraints>
- Do not assume pre-known hierarchy or key signals.
- Identify signals and control patterns automatically.
- Keep summary concise (bullet form).
- Provide a clean checklist table.
</constraints>

<think>
1. Parse RTL chunk; identify I/O, internal signals.
2. Trace primary data/control paths.
3. List instantiated submodules.
4. Initialize checklist with each submodule marked Pending.
5. Prompt user for next submodule's RTL chunk.
</think>
...
|------------------|-------------------------------|----------|
| childA           | e.g., data decoder/encryptor | Pending |
| childB           | e.g., FSM/controller         | Pending |
| childC           | ...                           | Pending |

**Next Step**  
Please select one submodule from the checklist and provide its RTL chunk.
</format>
...
<goal>
Update the checklist and analyze the selected submodule’s RTL chunk.
</goal>

<task>
Using the RTL chunk provided and the existing checklist:
1. Summarize the submodule’s dataflow/function.
2. Detect further instantiated sub-submodules.
3. Update the checklist:
   • Mark current submodule as “Done”
   • Add any new sub-submodules as “Pending”
4. Ask the user to select the next “Pending” item and provide its RTL chunk.
</task>
...
<constraints>
- Auto-identify signals and control flow.
- Keep bullet-style summary; concise.
- Update checklist cleanly.
</constraints>

<think>
1. Review existing checklist and summary context.
2. Parse current RTL chunk for I/O, data/control paths.
3. Identify instantiated lower modules.
4. Update checklist: current → Done; add new ones as Pending.
5. Ask user to pick next.
</think>
...
| Submodule         | Role                      | Status   |
|------------------|---------------------------|----------|
| childA           | description from analysis | Done     |
| childB           | …                         | Pending  |
| childC           | …                         | Pending  |
| childA1          | new sub-submodule         | Pending  |
| childA2          | another sub-submodule     | Pending  |

**Next Step**  
Please choose one submodule from the updated checklist and provide its RTL chunk, or type “none” if no further submodules.
</format>
