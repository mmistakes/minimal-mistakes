...
def parse_markdown_sections(text: str) -> Dict[str, str]:
    ...
    Parse markdown text into up to five sections, where two middle sections are optional:
      - chunk_summary:        from **Chunk Summary** to the first of
                              **Detected Submodules**, **Updated Checklist**, or **Files to Save**
      - detected_submodules:  from **Detected Submodules** to the next of
                              **Updated Checklist** or **Files to Save** (empty if absent)
      - checklist:            from **Updated Checklist** to **Files to Save** (empty if absent)
      - file_names:           from **Files to Save** to **Next Step**

    Raises:
        ValueError: if any mandatory marker
                    (**Chunk Summary**, **Files to Save**, **Next Step**) is missing
                    or out of order, or if any optional marker appears
                    outside the chunk_summary–file_names range.
    """
    # Define markers
    M = {
    ...
    # Locate all markers with find() so we can check for missing ones
    idx_chunk   = text.find(M["chunk_summary"])
    idx_ds      = text.find(M["detected_submodules"])
    idx_updated = text.find(M["checklist"])
    idx_files   = text.find(M["file_names"])
    idx_next    = text.find(M["next_step"])

    # Check for missing mandatory markers
    missing = []
    if idx_chunk   == -1: missing.append(M["chunk_summary"])
    if idx_files   == -1: missing.append(M["file_names"])
    if idx_next    == -1: missing.append(M["next_step"])
    if missing:
        raise ValueError(f"Missing mandatory marker(s): {', '.join(missing)}")

    # Ensure mandatory markers are in the correct order
    if not (idx_chunk < idx_files < idx_next):
        raise ValueError(
            f"Markers out of order: expected "
            f"**Chunk Summary** < **Files to Save** < **Next Step**; "
            f"got positions {idx_chunk}, {idx_files}, {idx_next}"
        )

    # Validate optional markers, if present, lie between chunk_summary and file_names
    ...
    # Build a list of sections to extract, in order of appearance
    sections = [("chunk_summary", idx_chunk, len(M["chunk_summary"]))]
    if idx_ds != -1:
    ...
    # Slice out each section's content up to the next marker (or next_step)
    result: Dict[str, str] = {}
    for i, (key, start, mark_len) in enumerate(sections):
        end = sections[i+1][1] if i+1 < len(sections) else idx_next
        result[key] = text[start + mark_len : end].strip()

    # Ensure optional keys always exist
    result.setdefault("detected_submodules", "")
...
class RTLProcessor:
    def __init__(self, rtlDir: str):
        self.rtlDir = rtlDir
        self.rtlList: List[str] = []
        self.num_modules: int = 0

    def chunk_rtl(
        self,
        file_name: str,
        max_chunk_size_kb: int = 40,
        overlap_ratio: float = 0.1
    ) -> None:
        """
        file_name 파일을 max_chunk_size_kb KB 단위로 분할하여
        self.rtlDir/<basename>/ 하위에 저장합니다.
        각 청크 파일의 맨 앞에는 '// chunk N out of M' 주석이 추가됩니다.
        청크 간에는 overlap_ratio 비율만큼(예: 0.1 = 10%) 겹칩니다.
        """
        # 1) 실제 파일 크기 확인 (KB, 올림)
        size_bytes = os.path.getsize(file_name)
        size_kb = size_bytes // 1024 + 1
        if size_kb <= max_chunk_size_kb:
            # 분할 필요 없으면 바로 종료
            return

        # 2) 출력 디렉터리 준비
        base = os.path.splitext(os.path.basename(file_name))[0]
        out_dir = os.path.join(self.rtlDir, base)
        os.makedirs(out_dir, exist_ok=True)

        # 3) 분할 단위 및 오버랩 계산
        chunk_bytes   = max_chunk_size_kb * 1024
        overlap_bytes = int(chunk_bytes * overlap_ratio)
        step_bytes    = chunk_bytes - overlap_bytes

        # 4) 전체 청크 개수 계산
        total_len   = size_bytes
        total_chunks = math.ceil((total_len - chunk_bytes) / step_bytes) + 1
        ...
        for part in range(1, total_chunks + 1):
            ...
            chunk_fname = f"{base}_chunk{part}.v"
            chunk_path  = os.path.join(out_dir, chunk_fname)

            # 6) 청크 파일에 주석과 데이터 쓰기
            comment = f"// chunk {part} out of {total_chunks}\n".encode('utf-8')
...
```
def append_detected_submodules(
    self,
    detected_submodules: str,
    csv_path: str = "module_file_list.csv"
) -> None:
    """
    detected_submodules 섹션에서 모듈명을 파싱하고,
    module_file_list.csv (module_name,file_name,file_size(in KB))를 읽어서:
      - file_size > 40KB : self.chunk_rtl(file_name) 호출
      - file_size <= 40KB : self.rtlList에 추가
    처리 후 self.num_modules를 self.rtlList의 길이로 설정합니다.
    """
    import csv

    # 1) detected_submodules에서 모듈명만 추출
    module_names = []
    for line in detected_submodules.splitlines():
        name = line.strip().lstrip("-* ").strip()
        if name:
            module_names.append(name)

    # 2) CSV에서 module→(file_name, size_kb) 매핑 읽기
    mapping = {}
    with open(csv_path, newline="", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            mod = row["module_name"].strip()
            file_name = row["file_name"].strip()
            try:
                size_kb = int(row["file_size(in KB)"])
            except (KeyError, ValueError):
                raise ValueError(f"Invalid size in CSV for module '{mod}': {row.get('file_size(in KB)')}")
            mapping[mod] = (file_name, size_kb)

    # 3) 각 모듈에 대해 chunk 여부 판정 및 처리
    for mod in module_names:
        entry = mapping.get(mod)
        if not entry:
            print(f"[WARN] Module '{mod}' not found in {csv_path}")
            continue

        file_name, size_kb = entry
        if size_kb > 40:
            # 40KB 초과 시 chunking
            self.chunk_rtl(file_name)
        else:
            # 아닐 경우 리스트에 추가
            self.rtlList.append(file_name)

    # 4) 최종 모듈 수 업데이트
    self.num_modules = len(self.rtlList)
...
#!/usr/bin/env bash
#
# Script: generate_module_file_list.sh
#
# 설명:
#   지정한 RTL 디렉터리 내의 모든 .v 파일을 검사하여,
#   각 파일에서 선언된 module 이름과 파일명, 파일 크기(KB)를 추출한 뒤
#   CSV 형태로 저장합니다.
#
# 사용법:
#   ./generate_module_file_list.sh <RTL_DIR> <DST_DIR> [RESULT_F_NAME]
#
#   <RTL_DIR>         : .v 파일들이 들어있는 디렉터리 경로
#   <DST_DIR>         : 결과 CSV를 저장할 디렉터리 경로
#   [RESULT_F_NAME]   : (선택) 결과 파일명, 기본값은 module_file_list
#
# 출력:
#   ${DST_DIR}/${RESULT_F_NAME}.csv
#   — CSV 헤더: module_name,file_name,file_size(in KB)
#   — 각 모듈마다 한 줄씩 “모듈명,파일명,파일크기” 형태로 기록
#
# 예시:
#   ./generate_module_file_list.sh ./rtl ./output
#   → ./output/module_file_list.csv 생성
#
...
# 인자 파싱
if [[ $# -lt 2 || $# -gt 3 ]]; then
  echo "Usage: $0 <RTL_DIR> <DST_DIR> [RESULT_F_NAME]" >&2
  exit 1
fi

RTL_DIR=$1
DST_DIR=$2
RESULT_F_NAME=${3:-module_file_list}

# 출력 디렉터리 및 파일 준비
mkdir -p "$DST_DIR"
OUTPUT_FILE="$DST_DIR/${RESULT_F_NAME}.csv"

# CSV 헤더 작성
echo "module_name,file_name,file_size(in KB)" > "$OUTPUT_FILE"
...
for f in "$RTL_DIR"/*.v; do
  ...
  # 파일 크기(KB) 계산 (du -k: KB 단위, cut -f1: 크기만)
  size_kb=$(du -k "$f" | cut -f1)

  # 해당 파일에서 module 선언부만 추출하여 각 모듈별로 CSV 라인 추가
  grep -hPo '^\s*module\s+\K[a-zA-Z_]\w*' "$f" | while read -r mod; do
    echo "$mod,$(basename "$f"),$size_kb" >> "$OUTPUT_FILE"
  done
done
...
def parse_markdown_sections(text: str) -> Dict[str, str]:
    ...
    Raises:
        ValueError: if any of the mandatory markers
                    ...
                    appears outside the chunk_summary–file_names range.
    """
    # Define markers
    M = {
        "chunk_summary":       "**Chunk Summary**",
...
if __name__ == "__main__":
    ...
    **Chunk Summary**
    - 이곳에 청크 요약 내용이 들어갑니다.
    여러 줄일 수도 있고, _markdown_ 형식이 섞여 있을 수도 있죠.
...
```
flowchart LR
  A[Prompt A: Top Module 분석] --> B{"Prompt B: Submodule 분석 반복"}
  B --> C[Prompt B: 다음 Submodule]
  C --> B
  B --> D[Prompt C: 최종 IP Dataflow 시각화]

```

Hierarchical chunking 개념도
```
flowchart LR
...
```
flowchart TD
  subgraph Stage_A ["Prompt A: Top Module 분석"]
    A1[Input: Top module chunk(s)]
    A2{Chunks > 1?}
    A3[Loop: summarize each chunk]
    A4[Combine chunk summaries → summary_top.md]
    A5[Generate checklist of submodules]
  end

  subgraph Stage_B ["Prompt B: Submodule 분석"]
  ...
  subgraph Stage_C ["Prompt C: 최종 IP Dataflow 분석"]
...
'''
Key Insights
	•	Dataflow: Inputs → processing → outputs
	•	Control: FSMs, enables, handshakes
	•	Power-saving: Techniques implemented per module
	•	Verification: Confirm all modules are included
</format>
