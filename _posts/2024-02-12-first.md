...
=========
```python
return re.sub(r'(?<![\d/])0([0-9]+)(?![/\d])', r'"\g<0>"', text)
```

import os, math, re

def sanitize_leading_zero_numbers(text: str) -> str:
    """
    입력 텍스트에서 0으로 시작하는 10진 숫자 리터럴을
    문자열로 감싸서 Python 해석 오류 예방.
    (예: 08 -> "08")
    """
    return re.sub(r'\b0([0-9]+)\b', r'"\g<0>"', text)

def chunk_file(path: str, max_bytes: int = 40960, overlap_ratio: float = 0.1):
    with open(path, 'r', encoding='utf-8', errors='ignore') as f:
        lines = f.readlines()

    total_size = os.path.getsize(path)
    if total_size <= max_bytes:
        text = sanitize_leading_zero_numbers(''.join(lines))
        return [(1, 1, text)]

    avg_line_size = total_size / max(len(lines), 1)
    lines_per_chunk = max(1, int(max_bytes / avg_line_size))
    overlap = max(1, int(lines_per_chunk * overlap_ratio))
    step = lines_per_chunk - overlap
    n_chunks = math.ceil((len(lines) - overlap) / step)
    ...
    for i in range(n_chunks):
        ...
        text = sanitize_leading_zero_numbers(''.join(chunk_lines))
        chunks.append((i + 1, n_chunks, text))
        if end >= len(lines):
            break
    return chunks

def save_chunks(chunks, original_path):
    base, _ = os.path.splitext(os.path.basename(original_path))
    for idx, total, content in chunks:
        fname = f"{base}_pt{idx}.v"
        with open(fname, 'w', encoding='utf-8') as f:
            f.write(f"// Chunk {idx} of {total} from {base}\n")
            f.write(content)
    print(f"Saved {len(chunks)} chunk files for '{base}'.")
