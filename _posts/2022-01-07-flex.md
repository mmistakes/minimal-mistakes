...
**inline-flex**는 block과 inline-blick의 관계라고 생각하면 된다. 아이템의 배치와 관련 있다기 보다는, 컨테이너가 주변 요소들과 어떻게 배치될지 결정하는 값이다. 즉 inline-flex는 inline-block처럼 동작한다.
...
- **column** 은아이템들이 열(세로) 방향으로 배치됩니다. (block 요소처럼)
- **column-reverse**은 아이템들이 역순으로 세로 배치 됩니다.
...
- **wrap-reverse**은 줄바꿈을 하는데, 아이템을 역순으로 배치한다. 
...
- **space-around**은 아이템들의 “둘레(around)”에 균일한 간격을 만들어 줍니다. 예를 들면 [  a    b    c  ]
