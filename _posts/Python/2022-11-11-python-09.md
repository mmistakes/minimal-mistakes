...
# ✔️ Data type
- <mark style="background-color: #DDDDDD">int</mark> : 정수 (음의 정수, 0, 양의 정수)
- <mark style="background-color: #DDDDDD">uint</mark> : 양수 (0, 양의 정수)
- <mark style="background-color: #DDDDDD">float</mark> : 실수 (부동 소수형)
- <mark style="background-color: #DDDDDD">complex</mark> : 복소수 (실수 + 허수)
- <mark style="background-color: #DDDDDD">bool</mark> : true, false
...
## 🔍 downcast 관련 함수
- pandas 의 ```to_numeric``` 함수로 정형 데이터에 대해 downcast를 적용할 수 있다.
  - <mark style="background-color: #DDDDDD">pandas.to_numeric<mark>
    - 인수를 숫자형으로 전환하는 기능
...
데이터는 컴퓨터 RAM 용량만큼 불러올 수 있는데, 대용량 데이터를 불러와서 분석하거나 모델을 만들기 위해서는 메모리를 효율적으로 사용할 수 있어야 한다. <mark style="background-color: #DDDDDD">downcast가 필요한 이유</mark>는 메모리 사용량을 절약해 더 많은 데이터를 불러오고 더 많은 데이터를 분석하기 위해서이다!
...
### (1) 데이터 기본 구조
- ```Survived```, ```Pclass```, ```SibSp```, ```Parch``` 컬럼은 모두 최솟값이 0, 최댓값이 10 이하인 데이터로 구성되어있다.
  - 하지만 dtype을 보면 모두 int64로 설정되어있다
  - 즉, RAM이 불필요하게 많이 쓰이고 있다는 뜻이다.
...
<img src="https://ifh.cc/g/xzzxly.png" width="500">
</div>
<center><small>데이터프레임의 정형 데이터에 대한 기술통계값</small></center>

<br>

<div style="text-align : center;">
<img src="https://ifh.cc/g/mZfSov.png" width="300">
</div>
<center><small>downcast 전, 데이터프레임 요약 정보</small></center>
...
# downcast 후 데이터프레임의 기본정보 확인
df.info()
```

<div style="text-align : center;">
<img src="https://ifh.cc/g/st0Bk4.png" width="300">
</div>
<center><small>downcast 후, 데이터프레임 요약 정보</small></center>
...
<div style="text-align : center;">
<img src="https://ifh.cc/g/yby9Qh.png" width="600">
</div>
...
# ✔️ 마치며
Kaggle의 train.csv 파일의 데이터를 불러왔을 때, 메모리 사용량을 보면 <mark style="background-color: #DDDDDD">83.5+KB</mark> 인데 정형 데이터를 pandas의 ```to_numeric``` 함수를 사용하여 낮은 버전의 dtype으로 바꿈으로써 메모리 사용량을 <mark style="background-color: #DDDDDD">52.2+KB</mark>로 약 <mark style="background-color: #DDDDDD">63%</mark> 줄여보았다.<br>
<br>정형 데이터의 범위에 비해 dtype으로 표현할 수 있는 숫자 범위가 크다면 downcast로 데이터 사용량을 절약할 수 있다. 즉, 표현 범위(메모리크기)에 맞는 형태를 쓴다면 메모리가 절약되고 좀 더 효율적으로 RAM을 사용할 수 있다.
