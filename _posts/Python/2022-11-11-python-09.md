...
## 🔍 pd.to_numeric
<mark style="background-color: #DDDDDD">기본 구조</mark> : ```pandas.to_numeric(arg, errors='raise', downcast=None)```
- 인수를 숫자형으로 전환하는 기능
- 기본적으로 반환되는 dtype은 데이터에 따라 float64, int64 이다.
- 다른 dtype을 설정하려면 downcast 매개변수를 이용하면 된다.

<mark style="background-color: #DDDDDD">주요 파라미터</mark>
...
데이터는 컴퓨터 RAM 용량만큼 불러올 수 있는데, 대용량 데이터를 불러와서 분석하거나 모델을 만들기 위해서는 메모리를 효율적으로 사용할 수 있어야 한다.<br>
<mark style="background-color: #DDDDDD">downcast가 필요한 이유</mark>는 메모리 사용량을 절약해 더 많은 데이터를 불러오고 더 많은 데이터를 분석하기 위해서이다!
