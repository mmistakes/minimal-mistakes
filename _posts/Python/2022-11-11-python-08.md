...
<small>Parquet 으로 빅데이터 파일 용량을 줄여 디스크 공간 절약하기 </small>
...
<br>복잡한 데이터를 대량으로 처리하기 위해 향상된 성능과 함께 효율적인 데이터 압축 및 인코딩 체계를 제공한다.<br>
<br>열의 값은 물리적으로 인접한 메모리 위치에 저장된다.
<br>열 단위 압축은 효율적이고 저장 공간을 절약한다.
<br>열 값이 동일한 데이터 타입이기 때문에 압축에 유리하다.
<br>특정 열 값을 가져오는 쿼리는 전체 행 데이터를 읽을 필요가 없으므로 성능이 향상된다.
<br>각 열에 다른 인코딩 기술을 적용할 수 있다.
...
## (1) Parquet 정의
CSV 및 JSON과 같은 행 기반 형식은 대부분 사람들이 읽을 수 있는 반면 열 기반 형식은 컴퓨터에 최적화 되어 있다. 컬럼 형식의 파일 형식은 다른 형식보다 훨씬 효율적이고 비용 효율적으로 컴퓨터에서 읽을 수 있다.
<br>빅데이터, 분석 및 데이터 레이크 스토리지에 이상적인 파일 형식이다.
<br>고성능이고 효율적인 압축이 가능하여 업계 표준 열 형식 파일이다.
...
## (3) Parquet 저장 및 읽기 방법
### 라이브러리 설치
- pandas에서 ```to_parguet``` 함수를 사용해서 데이터프레임을 parquet 포맷으로 변경할 때 사용할 수 있는 엔진의 옵션이 두 가지가 있다.
  - ```fastparquet```
  - ```pyarrow```
...
- 윈도우(Window) : anaconda prompt를 열어 설치, 터미널에 (base) 가상환경 표시 확인
- 맥(Mac) : terminal를 열어 설치, 터미널에 (base) 가상환경 표시 확인
...
- 출력 : {'parquet': '2.42 MB', 'csv': '13.06 MB'}
- parquet 로 저장하면 약 5배 정도의 저장 공간을 절약할 수 있다.

### csv와 parquet 파일로드 후, info 실행 시간 비교
...
<div style="text-align : center;">
<img src="https://ifh.cc/g/q0rG5h.png" width="400">
</div>
<center><small>Fig 3. csv의 파일 로드 후, info 실행 시간</small></center>
...
<div style="text-align : center;">
<img src="https://ifh.cc/g/0R9vBZ.png" width="400">
</div>
<center><small>Fig 4. parquet의 파일 로드 후, info 실행 시간</small></center>
