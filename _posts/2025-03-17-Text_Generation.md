...
처음 문장($x = x_1, ..., x_k$)이 주어질 때, 텍스트에 등장하는 토큰 시퀀스 ($y = y_1, ..., y_t$)의 확률 $P(y|x)$를 추정하도록 사전 훈련됩니다.

하지만 직접 $P(y|x)$을 추정하려면 방대한 양의 훈련 데이터가 필요하므로 **연쇄법칙(Chain Rule of Probability)** 을 사용해 조건부 확률의 곱으로 나타냅니다.

$$
P(y_1, ..., y_t | x) = \prod_{t=1}^{N} P(y_t | y_{<t}, x)
$$

이렇게 계산된 확률을 기반으로, 각 시점 $t$에서 가장 확률이 높은 단어를 선택하여 다음 단어를 예측합니다.
