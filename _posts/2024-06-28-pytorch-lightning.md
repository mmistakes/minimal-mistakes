...
솔직히 말하면 굳이 사용하지 않아도 괜찮을 것이다. 새로운 라이브러리를 사용하는데는 언제나 그렇듯 기본적인 이해도를 갖추는 시간이 필요하고, 손발 다루듯 하려면 이래저래 삽질을 많이 해야한다. 그럼에도 불구하고 나는 Pytorch Lightning과 Hydra만큼은 딥러닝 개발에 있어서 활용도가 높다고 느꼈다.
