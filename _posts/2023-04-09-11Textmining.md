...
- **원하는 기호 처리** : 위에서 \[^a-zA-Z]을 통해 모든 알파벳만 제외시켰다면, 이번에는 ()[]\/@:;.!? 등 원하는 기호를 [] 괄호 안에 넣어 공백(" ")으로 바꿔주었습니다.<br>이러한 방법을 이용하면 전처리가 되지 않았던 기호를 확인해 추가하는 방식으로 사용할 수 있습니다.<br>~~*실제 구현결과 실행결과가 제대로 안 나오지만 강의에 있는 자료이기에 포함시켰다. 추후에 업데이트 예정~~
