...
// I want to delete properties other than 'skills'
const txtOnlySkills = JSON.parse(txt, (key, value) => {
  let txtNames = Object.keys(JSON.parse(txt));
  for (let i = 0; i < txtNames.length; i++) {
    console.log(JSON.parse(txt)[`${txtNames[i]}`][1]);
  }
});
```
