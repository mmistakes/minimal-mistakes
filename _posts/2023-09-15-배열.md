...
여기서 주의할 점은 데이터들이 흩어져서 저장되는 것이 아니라 연속적인 공간들을 차지하면서 저장된다는 점이다.</span> 만약 배열의 크기가 원소의 개수보다 크다면 그 차이만큼의 원소는 0들로 채워지고 추후에 데이터가 추가되면 맨 앞의 비워 있던(0으로 표시해둔) 공간부터 데이터가 추가된다.<br>
<br>
<span style = "font-weight:bold;">
...
<br>
<span style = "font-weight:bold;">
1) 가변 크기: 동적 배열(리스트)은 상황에 따라 자동으로 크기가 조절된다. 즉, 정적 배열과는 달리 데이터를 자유롭게 추가하거나 삭제하는 것이 가능하다.</span> 만약 배열의 공간이 꽉 찬 상태에서 데이터가 추가된다면 프로그래밍 언어에서 자동으로 더 큰 공간을 가진 새로운 배열을 만들고 기존에 있던 데이터를 복사해서 새로운 배열로 옮긴 후에 새로운 데이터를 추가하는 방식이다. 예를 들어 파이썬에 [1,2,3,4,5]라는 리스트를 생성한 후 6이라는 원소를 추가하고 싶다면 자동으로 기존의 리스트보다 n배(예시 수치)만큼 더 큰 배열을 만들고 기존 원소인 1,2,3,4,5를 복사하여 옮긴 후 새로운 원소인 6을 추가하는 방식이다. 배열에 여유 공간이 남아 있다면 그냥 새로운 데이터를 추가하면 된다. 처음부터 많은 메모리 공간을 가진 리스트로 만들지 않는 이유는 공간의 낭비를  최소화하기 위해서지만 그래도 불가피하게 정적 배열과는 달리 낭비되는 메모리 공간들이 발생할 수 있다는 단점이 존재한다.<br>
<br>
<span style = "font-weight:bold;">
