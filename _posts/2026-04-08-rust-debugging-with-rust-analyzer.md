...
이 글에서 설명하는 방식은 반드시 `cargo new 프로젝트명`으로 만든 Cargo 프로젝트 기준이다. VS Code Rust 문서는 Cargo 프로젝트와 `rust-analyzer` 기반 디버깅 흐름을 기준으로 설명한다([Rust in Visual Studio Code](https://code.visualstudio.com/docs/languages/rust)).
...
VS Code의 확장 메뉴에서 `rust-analyzer`를 설치한다. VS Code Rust 공식 문서는 `rust-analyzer` 설치를 기본 흐름으로 안내한다([Rust in Visual Studio Code](https://code.visualstudio.com/docs/languages/rust); [rust-analyzer book](https://rust-analyzer.github.io/book/)).

![rust-analyzer]({{ '/images/rust_02/rust-analyzer.png' | relative_url }})

직접 확인: 작성 환경에서 Cargo 프로젝트를 VS Code로 열면 `fn main()` 위쪽에 `Run | Debug` 버튼이 나타났다.

![Run Debug]({{ '/images/rust_02/run_debug_%EB%B2%84%ED%8A%BC.png' | relative_url }})

여기서 `Debug`를 클릭하면 바로 디버깅 모드로 들어갈 수 있다. VS Code의 Rust 공식 문서도 `rust-analyzer` 확장을 설치한 뒤 `Run | Debug` CodeLens 또는 `Rust Analyzer: Debug` 명령으로 디버깅을 시작하는 흐름을 안내한다([Rust in Visual Studio Code](https://code.visualstudio.com/docs/languages/rust)).

## CodeLLDB 설치

실제 디버깅 기능을 더 안정적으로 사용하려면 `CodeLLDB` 확장도 함께 설치하는 것이 좋다.
의견: 이 글은 Windows 예시도 `CodeLLDB` 기준으로 설명한다.

![CodeLLDB install]({{ '/images/rust_02/codeLLDB%EC%84%A4%EC%B9%98.png' | relative_url }})

`CodeLLDB`는 LLDB 기반 디버거 확장으로, Rust 같은 네이티브 언어를 VS Code에서 디버깅할 때 자주 사용된다. 2026-04-15 기준 VS Code Rust 공식 문서는 Windows에서는 Microsoft C++ 확장, macOS/Linux에서는 `CodeLLDB`를 먼저 안내한다. `CodeLLDB` 자체 기능은 원저자 저장소에서 확인할 수 있다([Rust in Visual Studio Code](https://code.visualstudio.com/docs/languages/rust); [CodeLLDB repository](https://github.com/vadimcn/codelldb)).
...
`launch.json`은 VS Code가 디버깅을 시작할 때 어떤 디버거를 사용할지, 어떤 실행 파일을 띄울지, 현재 작업 폴더는 어디인지, 인자를 무엇으로 넘길지를 저장하는 설정 파일이다. VS Code 공식 문서는 복잡한 디버깅 시나리오에서 `.vscode/launch.json`으로 디버거 구성, 실행 파일, 작업 디렉터리, 인자를 지정하도록 설명한다([Visual Studio Code debug configuration](https://code.visualstudio.com/docs/debugtest/debugging-configuration)).
...
실제로는 `F9`로 멈출 위치를 잡고, `F5`로 실행한 다음 `F10`, `F11`을 섞어서 흐름을 보면 가장 이해하기 쉽다. 이 키 설명은 VS Code 디버깅 문서의 기본 단축키 설명과 맞닿아 있다([Debug code with Visual Studio Code](https://code.visualstudio.com/docs/debugtest/debugging)).
