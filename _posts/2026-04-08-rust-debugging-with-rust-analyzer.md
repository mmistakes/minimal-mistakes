...
근거: VS Code Rust 문서는 Cargo 프로젝트와 `rust-analyzer` 기반 디버깅 흐름을 기준으로 설명한다([Rust in Visual Studio Code](https://code.visualstudio.com/docs/languages/rust)).
...
근거: VS Code Rust 공식 문서는 `rust-analyzer` 설치를 기본 흐름으로 안내한다([Rust in Visual Studio Code](https://code.visualstudio.com/docs/languages/rust); [rust-analyzer book](https://rust-analyzer.github.io/book/)).
...
근거: VS Code의 Rust 공식 문서도 `rust-analyzer` 확장을 설치한 뒤 `Run | Debug` CodeLens 또는 `Rust Analyzer: Debug` 명령으로 디버깅을 시작하는 흐름을 안내한다([Rust in Visual Studio Code](https://code.visualstudio.com/docs/languages/rust)).
...
근거: 2026-04-15 기준 VS Code Rust 공식 문서는 Windows에서는 Microsoft C++ 확장, macOS/Linux에서는 `CodeLLDB`를 먼저 안내한다. `CodeLLDB` 자체 기능은 원저자 저장소에서 확인할 수 있다([Rust in Visual Studio Code](https://code.visualstudio.com/docs/languages/rust); [CodeLLDB repository](https://github.com/vadimcn/codelldb)).
...
근거: VS Code 공식 문서는 복잡한 디버깅 시나리오에서 `.vscode/launch.json`으로 디버거 구성, 실행 파일, 작업 디렉터리, 인자를 지정하도록 설명한다([Visual Studio Code debug configuration](https://code.visualstudio.com/docs/debugtest/debugging-configuration)).
...
근거: 이 키 설명은 VS Code 디버깅 문서의 기본 단축키 설명과 맞닿아 있다([Debug code with Visual Studio Code](https://code.visualstudio.com/docs/debugtest/debugging)).
