...
## 예시로 이해하기
- 라이어 게임 (참가자 5명 - A, B, C, D, E)
  - Case 1. 첫 번째 라이어 지목에서, A가 3표, C가 1표, D가 1표를 받았습니다.
    - 그럼 아래의 엔트로피 계산식에 의해 ```1.37``` 라는 값이 나옵니다.
    - 최대 엔트로피 수 : ```log2(5) = 2.32```

  - Case 2. 두 번째 라이어 지목에서, C가 4표, E가 1표를 받았습니다.
    - 그럼 아래의 엔트로피 계산식에 의해 ```0.72``` 라는 값이 나옵니다.
    - 최대 엔트로피 수 : ```log2(5) = 2.32```
