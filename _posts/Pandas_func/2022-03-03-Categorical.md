...
🌵 가볍게 Categoical 함수에 대해서 살펴봤다. 함수 내에 일일이 카테고리 리스트를 순서지어 넣어줘야한다는 점이 방대한 데이터 분석에는 약간 걸리지만 datetime 함수와 엮어서 사용할 때는 훨씬 깔끔한 결과를 얻을 수 있다고 생각한다. 정렬을 해도 원하는 결과가 안 나올때는 이 함수를 써보는 건 어떨까😊!!
