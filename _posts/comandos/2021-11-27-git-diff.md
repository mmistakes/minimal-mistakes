...
* Manual :
  * ``git diff`` → Los cambios en el **(Working Tree)** no están aún preparados **{UnStaged}** para la próxima confirmación ``git commit``
...
* ``git diff HEAD^ HEAD`` → Compara la versión antes del último commit con el último commit
...
2-. y ``Changes not staged for commit:`` lo que significa que hay cambios
sin confirmar debido a los últimos cambios realizados dentro del archivo y que no hemos añadido al **{Staging Area/INDEX}**
...
![Alt Resumen escrito a papel](/assets/images/diff/resumen-diff.jpeg "Resumen escrito a papel")
