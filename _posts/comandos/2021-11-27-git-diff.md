...
* ``git diff`` → Este comando compara lo que esta su **(Working Directory/WorkSpace)** con lo que esta en el **{Staging Area/INDEX}** , el resultado te dice los cambios que has hecho pero que todavía no has preparado **{Not-Staged}** mediante el comando ``git add <archive>``

  * Muestra las diferencias no añadidas al **{INDEX}** mediante ``git add <archivo>``

* ``git diff <commit>`` → Muestra los cambios que existen en el **(Working Directory/WorkSpace)** relativos al commit seleccionado

* ``git diff`` → Por sí mismo no muestra todos los cambios realizados desde el **último commit** , sólo los cambios que aún no han sido ``{Staged-Preparados}``

* ``git diff LICENCE`` → Muestra solo los cambios en el archivo **LICENCE** pero sólo si ha sido modificado

* ``git diff --staged`` → Si quieres ver lo que has preparado en el estado **{Staged}** para en tu próximo **commit** , este comando compara sus cambios {Staged-Preparados} con su **último commit**
...
Comparar los cambios efectuados **(Working Directory)** con respecto al **{Staging Area/INDEX}** mostrará los cambios que todavía no se han preparado **{UnStaged}** , git ejecutará la comparación con respecto a **{HEAD}** , en el caso de omitirse tendrá el mismo efecto que si ejecutamos ``git diff --cached <archivo>``
