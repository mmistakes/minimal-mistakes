...
* ``git diff`` → Este comando compara lo que esta su **(Working Directory)** con lo que esta en el **{Staging Area}** , el resultado te dice los cambios que has hecho pero que todavía no has preparado **{Not-Staged}** mediante el comando ``git add <archive>``
