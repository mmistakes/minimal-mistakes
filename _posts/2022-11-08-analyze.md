...
## 6. 컨브넷이 학습한 것 해석하기  
 ...
 사람들은 딥러닝 모델을 '블랙박스' 같다고 자주 이야기한다. 모델이 학습한 표현을 사람이 이해하기 쉬운 형태로 뽑아내는 것이 어렵기 때문이다. 하지만 이는 컨브넷 모델에서는 틀린말이  다. 컨브넷의 표현은 시각적인 개념을 학습한 것이기 때문에 시각화에 용이하다. 이번 글에서는 컨브넷의 표현들을 시각화하고 해석하는 다음의 세가지 기법을 알아보겠다. 
