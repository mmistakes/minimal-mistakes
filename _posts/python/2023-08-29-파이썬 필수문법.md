...
1) 인스턴스는 클래스가 코드상에 존재할수 있게하여 사용가능 하도록 할때 사용한다.
2) 파이썬에서는 자료형도 클래스이다.
...
1) init 생성자는 인스턴스 만들때 반드시 호출되는 메서드이다
2) 인스턴스=클래스() : 이렇게 가로 닫힐때 init이 활성화되어 호출이 될 수 있다.
3) 생성자 : 인스턴스를 만들때 생성되는 메서드를 지칭한다.

4) 객체는 인스턴스를 포함한다.

- self 란?

1) __init__(self) : self는 인스턴스의 자기자신을 가르키게 된다.
2) self.name = name : 현재 호출되어진 인스턴스 자리에 name변수에다가 name으로 대체하라는 뜻이다.
