...
수학 함수로도 구할 수 있지만 통계 함수는 한번에 정확한 값을 효율적으로 계산해낸다. 통계는 여러개의 값을오부터 원하는 값을 찾아내는 것이므로 인수로 수치값의 리스트가 온다. 입력값이 많아도 빠른 속도로 통계값을 계산해 낸다.  
...
1679758311.265318
```
현재 시간으로 조사하는 time함수이다. 유닉스는 1970년 1월 1일 자정을 기준으로 경과한 시간을 초 단위로 표현하는데, 이를 Epoch시간 또는 유닉스 시간이라고 부른다. 현재 시간을 조사해보면 16억정도의 큰 값이 나오는데, 1970년 부터 지금까지 그정도의 초가 지났다느느 이야기이다. 시간을 딱 하나의 값으로 1차원화 하여 표현할수 있어서 계산이나 저장이 편하고, 다른 시스템과 통신에 유리하다.
