...
두번째로 시도한 방식은 결측치가 많은 컬럼을 삭제하는 것이었다. 이 방법이 꽤나 유효했는데 실제 데이터셋이 가진 컬럼들을 봤을 때 "해제사유발생일", "등기신청일자", "단지소개기존clob" 과 같이 부동산 가격에 영향을 주지 않을 것이라 생각되는 변수들이 다수 존재하였으며 이러한 변수들이 결측치를 갖는 경우가 대부분이다.

<img src="{{site.url}}/images/240719/0003.png" width="1920" height="1080">

이러한 해석이 객관적으로 올바른지 검증하고자 상관관계 분석을 진행했고, 행렬을 봤을 때 얻은 결론은 다음과 같다.

1. ```전체 동수``` 컬럼과 ```전체 세대수```, ```연면적```, ```주거전용면적```, ```관리비부과면적```, ```전용면적별세대현황```, ```주차대수```들과 높은 상관관계를 보인다.
2. 135m초과 컬럼은 다른 변수들과 아예 상관관계가 없다.
3. target은 ```전용면적```, ```계약년월```, ```연면적```, ```주거전용면적```, ```관리비부과면적```과 높은 상관관계를 보인다.
...
모든 아파트가 그런 것은 아니지만 시공사에 따라 아파트에 브랜드가 부여되어 프리미엄가격이 책정되기도 한다. 따라서 데이터셋에 있는 건설사별로 시공한 건물가격의 평균가를 기준으로 랭크를 부여했다. 즉, 동일한 스펙을 가진 부동산이라 하더라도 어떤 건설사가 시공했는지에 따라 적용되는 브랜드가격이 추가 가격으로 반영될 것이며 이는 곧 어느정도의 단가를 결정하는데 도움이 줄 것이라 판단했다.
...
- RandomSearch, Grid Search를 이용해 최적의 하이퍼파라미터 조합을 찾으려고 했다.
- 리더보드 RMSE : 24397.6060
...
- 안타깝지만 다른 팀원들이 가능한 제출 횟수를 모두 사용해버려서 이 학습 전략의 결과는 알 수가 없었다.
