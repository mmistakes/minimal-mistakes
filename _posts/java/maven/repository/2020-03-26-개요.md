...
기존에는 개발에 필요한 라이브러리는 프로그래머가 직접 다운받아 프로젝트에 추가했었다.  

이제는 **pom.xml에 기술**함으로서 쉽게 라이브러리를 사용할 수 있게 되었다.  **메이븐 중앙 저장소** 덕분이다.  

저장소는 일종의 라이브러리 보관 장소(=프로그램 보관 장소)이다.

pom.xml에 자신이 라이브러리를 받아올 주소를 지정해 원하는 저장소로 지정 가능하지만 아무것도 지정하지 않으면 **기본값으로 [메이븐 중앙 저장소](https://repo1.maven.org/maven2/)**이 지정된다.  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTg1MDI4Mzg1OF19
-->