...
* ``getLocalizedMessage()``
  * Realiza la misma acción que getMessage() a no ser que una clase hija lo haya sobreescrito para producir un mensaje especifico en una clase Locale en concreto
...
* ``printStackTrace()``
  * Muestra por pantalla la salida de error estandar (terminal o archivo logs) la ``stack trace`` (la pila de trazas) de la excepción
