...
위의 id = 2번 단계에서 모든 고객에 대한 합계를 계산하는 작업은 상당히 비효율적입니다. 왜냐하면 Derived Table(인라인뷰) 바깥의 조건절로 인해 필터링 될 것이기 때문에 고객 2명("Customer#1", "Customer#2")의 합산 결과만 알면 되기 때문입니다. 하지만 Derived Table 바깥에 해당하는 필터조건을 인라인뷰는 알 수 없기 때문에 불필요한 많은 연산 작업을 동반하게 됩니다.(고객테이블의 건수가 많으면 많을 수록 성능 부하는 심해집니다.)
