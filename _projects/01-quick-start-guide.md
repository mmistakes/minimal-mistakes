...
permalink: /projects/quick-start-guide/
...
Minimal Mistakes has been developed as a [Gem-based theme](http://jekyllrb.com/projects/themes/) for easier use, and 100% compatible with GitHub Pages when used as a remote theme.
...
[^structure]: See [**Structure** page]({{ "/projects/structure/" | relative_url }}) for a list of theme files and what they do.

**ProTip:** Be sure to remove `/projects` and `/test` if you forked Minimal Mistakes. These folders contain documentation and test pages for the theme and you probably don't want them littering up your repo.
{: .notice--info}
...
[ci-jekyll]: https://jekyllrb.com/projects/continuous-integration/
...
**Note:** If forking the theme be sure to update `Gemfile` as well. The one found at the root of the project is for building the theme's Ruby gem and is missing dependencies. To properly setup a [`Gemfile`](https://github.com/mmistakes/minimal-mistakes/blob/master/projects/Gemfile) with the theme, consult the "[Install Dependencies](https://mmistakes.github.io/minimal-mistakes/projects/installation/#install-dependencies)" section.
{: .notice--warning}
...
**ProTip:** The source code and content files for this site can be found in the [`/projects` folder](https://github.com/mmistakes/minimal-mistakes/tree/master/projects) if you want to copy or learn from them.
{: .notice--info}

### Starting Fresh

Starting with an empty folder and `Gemfile` you'll need to copy or re-create this [default `_config.yml`](https://github.com/mmistakes/minimal-mistakes/blob/master/_config.yml) file. For a full explanation of every setting be sure to read the [**Configuration**]({{ "/projects/configuration/" | relative_url }}) section.
...
- [`_data/ui-text.yml`](https://github.com/mmistakes/minimal-mistakes/blob/master/_data/ui-text.yml) - UI text [documentation]({{ "/projects/ui-text/" | relative_url }})
- [`_data/navigation.yml`](https://github.com/mmistakes/minimal-mistakes/blob/master/_data/navigation.yml) - navigation [documentation]({{ "/projects/navigation/" | relative_url }})
...
- Replace `<site root>/index.md` with a modified [Minimal Mistakes `index.html`](https://github.com/mmistakes/minimal-mistakes/blob/master/index.html). Be sure to enable pagination if using the [`home` layout]({{ "/projects/layouts/#home-page" | relative_url }}) by adding the necessary lines to **\_config.yml**.
...
You won't need these anymore as they're bundled with the theme gem --- unless you intend to [override them](http://jekyllrb.com/projects/themes/#overriding-theme-defaults).
...
If you customized any of these files leave them alone, and only remove the untouched ones. If done correctly your modified versions should [override](http://jekyllrb.com/projects/themes/#overriding-theme-defaults) the versions bundled with the theme and be used by Jekyll instead.
...
**v4 Breaking Change:** Paths for image headers, overlays, teasers, [galleries]({{ "/projects/helpers/#gallery" | relative_url }}), and [feature rows]({{ "/projects/helpers/#feature-row" | relative_url }}) have changed and now require a full path. Instead of just `image: filename.jpg` you'll need to use the full path eg: `image: /assets/images/filename.jpg`. The preferred location is now `/assets/images/` but can be placed elsewhere or externally hosted. This applies to image references in `_config.yml` and `author.yml` as well.
{: .notice--danger}
