...
(6LoWPAN
%\footnote{We use the acronym 6LoWPAN to refer to Low power
%Wireless Personal Area Networks that use IPv6}
) is a working group
...
%The advantage of a tree topology is a small routing table, since
%each node just needs to establish who his parent-node is and maintain only that
% information for packet forwarding. The disadvantage of such structures is that other communication patterns, like downward
%(P2MP) or any-to-any (P2P) bidirectional data flows, are not
%easily implemented.
...
%In order to do that, addition communication routines (downward
%(P2MP) or any-to-any (P2P) bidirectional data flows) have to be implemented and
%extensive routing information has to be inserted into the routing
%tables of memory-constrained nodes.
%Therefore, such standard protocols do not allow a straight-forward
% implementation of any-to-any communication patterns.
...
memory-constrained devices. %Our experiments show that in random topologies with
%one hundred nodes, with no link or node failures, RPL succeeds to
%deliver less than 20\% of top-down messages sent by the root (see
%Figure~\ref{fig:txdwn}).

Some works have addressed this problem from different perspectives.
%In \cite{Duque13}, ORPL, an extension of RPL, uses bloom
%filters and opportunistic routing to decrease control
%traffic overload. In \cite{Rein12}, point-to-point
%communication is enabled by having
%each node in a collection tree store the addresses of its direct and indirect
%child nodes using Bloom filters.
%In \cite{xctp}, XCTP, an extension of CTP is proposed, which uses
%opportunistic and reverse-path routing to enable bi-directional communication
% in CTP. XCTP is efficient in terms of message overload, but exhibits the problem of
%high memory footprint, since each node needs to store an entry in the local
%routing table for every data flow going through that node.
%As opposed to reactive strategies, such as ORPL and XCTP,
%in \cite{mhcl}, MHCL, a proactive approach based on hierarchical IPv6
%address allocation was implemented as a subroutine of RPL.
...
%MHCL not only makes every node adressable from outside the border router, but
%also allows for compact routing tables.
%In MHCL, the address space available to the border router, e.g. the 64
%least-significant bits of an IPv6 address (or a compressed 16-bit
% representation of the latter), is partitioned among nodes connected to the border router through
%a multihop cycle-free topology. Each node receives an address range from its
%parent and partitions it among its children, until all nodes receive an
% address.
%Since the address allocation is performed in a hierarchical way, each routing
%table entry aggregates the addresses of all nodes in the subtree rooted at the
%corresponding child-node.
%Even though MHCL greatly reduces routing table sizes needed for any-to-any
%traffic, it is limited in dealing with dynamic network topologies.
%, being able to handle only short link or node failures by using repeated
% message retransmissions.

%besides the collection tree (as in CTP) or DAG (as in RPL),
...
%Local broadcast is activated by a node when it fails to forward a message to
% the next hop (subtree) in the address hierarchy.
...
%, reflecting the dynamics of the collection tree in the reverse direction.
...
networks. Our simulations showed that this intuition is, in fact, correct. %In
%adverse network conditions, without the local broadcast mechanism, Matrix
%delivers 85\% of top-down messages when a route exists; with the local
%broadcast activated, the success rate increases to 95\% (roughly 2/3 of
%otherwise lost messages succeed in reaching the final destination).
%COMPARE TO MHCL NUMBERS
%The intuition behind this argument is based on the geometric properties of
% wireless networks, which bound the maximum number of independent neighbors of each node. 
%Our simulations that this intuition is, in fact, correct, and Matrix
%achieves $\geq 95\%$ TODO-CHECK end-to-end packet delivery in
%wireless low power networks with increasing failure rate and failure
%duration.
...
protocols, such as RPL. %, such as XCTP and RPL.
...
\begin{itemize}
  ...
  between hosts located within or outside the 6LoWPAN.% (e.g. with the source
  % host beyond the border router).
