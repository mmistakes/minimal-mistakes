...
However, we only sometimes want to change the type code to validate using subtyping, and sometimes we need help.
...
If we can't use subtyping, what other solutions do we have? We know this paradigm only has subtyping if we are functional programming geeks. However, we don't want to lose the possibility to write polymorphic code, such as one of the `process` functions above.
...
First, we must refactor the `Validatable<T>` interface. We'll proceed step by step, so the following version of the interface is not the final form:
...
We still miss a feature of the object-oriented solution: The `validate` method is not a DTO method. It's not a big problem, but it could be more elegant than the object-oriented solution. Fortunately, we can improve our solution in this direction, taking advantage of Kotlin extension functions. Let's do it.
...
Again, we used the receiver feature of the Kotlin language to access the `ValidatorScope<T>`. It's the caller's responsibility of the `process` function to provide the right `ValidatorScope<T>` instance. For example, we can call the `process` function as follows:
...
One open point about implementing type classes in Kotlin is that we still need an automatic discovery process. Other languages supporting type classes, such as Scala and Haskell, implement some form of automatic discovery. Scala, for example, has an implicit resolution.
...
The above validation functions require a validation rules type class to be available as an extension function on the type `T`. So, in this solution version, we need to use context receivers, since we can't have more than one receiver for a function.
