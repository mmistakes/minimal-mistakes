...
**1.** For a **new site**, install the `minimal-mistakes-jekyll` gem, remote theme, or fork the Minimal Mistakes repo on GitHub following the steps outlined in the [*About*]({{ "/docs/quick-start-guide/" | relative_url }}).
...
**2.** For an **existing site** follow the steps outlined in the [*About*]({{ "/docs/quick-start-guide/" | relative_url }}). Then work through the guidelines below for migration and setup.
