...
modified: 2017-01-30T06:32:43-04:00
redirect_from:
  - /theme-setup/
---
...
**ProTip:** Be sure to remove `/docs`, `/script` and `/test` if you forked Minimal Mistakes. These folders contain documentation test script and test pages for the theme and you probably don't littering up in your repo.
{: .notice--info}
...
Edit `_config.yml`:
  ...
  - `theme: minima` will need to be edited to `theme: minimal-mistakes-jekyll`
  - remove the `gems:` list. Plugins required by the theme will be called by the gem itself.
  If you do need to add extra plugins, you may add them to the `:jekyll_plugins` group in your `Gemfile`.

Edit `Gemfile`:

  - use the latest version of Minimal Mistakes : `gem "minimal-mistakes-jekyll", "~> 4.2"`
  - add `jekyll-data` plugin to the `:jekyll_plugins` group:
...
```
**Note:** `jekyll-feed` and other plugins required by the theme will be run by the theme-gem itself using the `jekyll-data` plugin and need not be included here. However, any other plugins that *you may need* for your site **should be added here** instead of the `gems:` list in your config file.
{: .notice--warning}

Once you've edited the config file and your Gemfile,
...
Start by removing `_data`, `_includes`, `_layouts`, `_sass`, `assets` folders and all files within. You won't need these anymore as they're bundled with the theme gem.
...
Replace `gem "github-pages` or `gem "jekyll"` with `gem "jekyll", "~> 3.3"`. You'll need the latest version of Jekyll[^update-jekyll] for Minimal Mistakes to work and load all of the theme's assets properly, this line forces Bundler to do that.
...
gem "minimal-mistakes-jekyll"
```
..and the `jekyll-data` plugin. Currently, source directly from the Github Repo:
...
Minimal Mistakes theme-gem uses the `jekyll-data` plugin to read-in data files as well as a `_config.yml` from within the theme-gem. This config file allows the theme to be run with default settings and is overridden by a `_config.yml` in your site.

At minimum, the config file in your site should contain the following..:

```yaml
theme: minimal-mistakes-jekyll
```
When using the `jekyll-data` plugin, the `gems:` array need not exist in your config file and can be safely deleted. If you need to add another plugin to your site, add them to the `:jekyll_plugins` group in your `Gemfile` and to the `whitelist` array in the config file.
