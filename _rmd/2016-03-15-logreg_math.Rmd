---
title: "logreg_cortez_math"
author: matt_gregory
comments: yes
date: '2016-03-01'
modified: `r format(Sys.time(), '%Y-%m-%d')`
layout: post
excerpt: "Predicting student performance using logistic regression"
published: yes
status: processed
tags:
- R
...
# Logistic regression to predict student end of year pass or failure
...
```
The `cv.glm()` function produces a list with several components. The two numbers in the `delta` vector contain the cross-validation results. Our cross-validation estimate for the test error is approximately `r round(cv.err$delta[1], digits = 3)`.
