...
```js
const { name: userDName, age, email, address, country = "Korea" } = userD;
// E.g user.address
// name의 값을 userDD라는 변수에 새로 할당 -> name은 이제 존재 X

console.log(`사용자의 이름은 ${userDName}입니다.`); // 그냥 name이라 하면 오류남!
console.log(`${userDName}의 나이는 ${age}세입니다.`);
console.log(`${userDName}의 이메일 주소는 ${email}입니다.`);
console.log(address); // undefined -> 비구조화 안에서 기본값 할당 가능
