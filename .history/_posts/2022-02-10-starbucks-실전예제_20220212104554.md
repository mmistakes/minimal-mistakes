...
- 스크롤하면 요소들 가운데로 오는 transition 효과랑 btn의 hover했을때 배경색 바뀌는 효과가 충돌하는걸 방지하려면?  
  => a태그를 div로 감싸면 됨
...
- 선택자를 그대로 써서 구현 가능
- 만약, 많이 쓰이는 요소의 선택자면, 변수에 한번 할당하여 그 변수를 계속 사용하는게 효율적!
...
- 기존의 main.js -> **루트경로의 index.html** 요소만 가져왔으므로, signin폴더안의 index.html 에서 main.js가져오면, 몇개의 요소들은 못 찾을수도 있음..
